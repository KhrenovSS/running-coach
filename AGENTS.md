...
## Текущее состояние (Session — 13.07.2026 — Модуль анализа + новый алгоритм интервалов)
...
**Алгоритм интервалов ✅:** Новый `src/analysis/oscillation.py` — детекция по базовому темпу (средний темп пробежки) + порог pace_gap. Настройки: порог ускорения (мин/км), мин. длительность фазы (сек), лаг пульса, мин. число осцилляций. `POST /session/{id}/reanalyze` — пересчёт с override типа. `trackpoints_json` для хранения трекпоинтов.

### Что сделано в этой сессии (13.07.2026):
1. Создан `src/analysis/` (6 файлов): `__init__.py` (оркестратор process_trackpoints), `oscillation.py` (base_pace + pace_gap детекция + HR-lag), `classify.py`, `segment.py`, `hr_zones.py`, `utils.py`
...
10. Новый алгоритм oscillation: `base_pace = mean(paces)`, `threshold = base_pace - pace_gap`, work = темп < threshold, recovery = темп >= threshold
