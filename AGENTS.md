...
- `src/services/repositories.py` — слой агрегационных запросов (TrainingRepository, HealthRepository) для модуля аналитики
...
- `src/services/user_service.py` — сервисные функции для User (get_user_settings, get_user_by_telegram_id, get_or_create_user_by_telegram, get_user_by_id)
- `src/coach/` — пакет модуля аналитики и коучинга (в разработке, Sprint 21+):
