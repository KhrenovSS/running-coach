...
**Sprint 20c ⬜:** Analytics Preparation — аудит выявил P0/P1 проблемы, блокирующие модуль аналитики: починить Telegram stats handler (несуществующие колонки), добавить индексы для time-range queries, создать слой агрегационных запросов (`src/services/repositories.py`), исправить двойную сериализацию `sleep_hrv_interval_list`, убрать хардкод `User.id == 1` из `get_settings()`, создать структурированные аналитические функции, добавить функции трендов (slope, EWMA, moving average), добавить `avg_pace` на `TrainingSession`, создать тестовые фабрики для DailyMetrics/TrainingSession, создать `src/coach/config.py`, разделить `src/models.py` на shim и сервисы, исправить форматирование темпа на графике (Chart.js tooltip/axis). 12 задач (PREP-01..PREP-11, PREP-17).
...
🚀 **Sprint 20c** — Analytics Preparation: починить Telegram stats handler (PREP-01), добавить индексы для time-range queries (PREP-02), создать слой агрегационных запросов (PREP-03), исправить двойную сериализацию HRV (PREP-04), убрать хардкод User.id==1 (PREP-05), создать структурированные аналитические функции (PREP-06), добавить функции трендов (PREP-07), добавить avg_pace на TrainingSession (PREP-08), создать тестовые фабрики (PREP-09), создать src/coach/config.py (PREP-10), разделить models.py на shim и сервисы (PREP-11), исправить форматирование темпа на графике (PREP-17). 12 задач. Детальное описание: `PROJECT_AUDIT.md` секция "Sprint 20c".
