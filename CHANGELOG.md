...
## [17.09.2026] — Разбор не называет тренировку вторника «вчерашней»: записи recent_reviews датированы по тренировке

Инцидент (прод, 19:30 МСК): разбор пробежки Чт 17.09 (спусков нет) закончил вопросом «ещё чувствуются вчерашние спуски?»
— вчера был отдых; спуски (`downhill_load_high`) и carry_forward «…объясняется спусками на маршруте…» — у тренировки
Вт 15.09. Блок `recent_reviews (workout_insights)` в контексте нёс только `session_id` и `days_ago` от `created_at`
строки разбора (не от даты тренировки), без `date`/`weekday`; правило персоны про «вчера» касалось лишь `days_ago`
тренировки — модель датировала заметку «себе на завтра» как вчерашнюю.

### Fixed
- **`recent_reviews` датированы по тренировке**: `InsightRepository.recent_with_sessions` (пары разбор + сессия;
  `recent` — тонкая обёртка), `turn_context.build_extras` кладёт `date`/`weekday`/`days_ago` от локальной даты
  тренировки (как `_session_brief`), не от `created_at` разбора (тот же дрейф давал бэкфилл).
- **Промпт**: `SYSTEM_PERSONA` — относительные даты по `days_ago` той записи, о которой речь (тренировка или прошлый
  разбор); `days_ago ≠ 1` → не «вчера», а день недели из `weekday`. `REVIEW_PROMPT` — `recent_reviews` описывает
  ПРЕЖНИЕ тренировки (сегодняшняя — только в `workout_detail`/`workout_computed`).

### Tests
- `tests/coach/test_influence.py`: `days_ago` записи разбора — от даты тренировки (2, не 0 при свежем разборе старой
  тренировки), `date`/`weekday` присутствуют в контексте утра и разбора.

### Backlog
- #358: `numeric_check` не проверяет относительные даты и дни недели в прозе («вчера», «во вторник») против
  `days_ago`/`weekday` контекста — гвард по аналогии с числами.

## [17.09.2026] — Синк Coros: Track Run (sportType 103) не забирался, кнопка «здоровье» на веб-форме падала (инцидент 17.09)

Инцидент (прод): тренировка 17.09 16:47 МСК режимом **Track Run (`sportType=103`)** не синхронизировалась ни авто-синком,
ни кнопкой на веб-форме — фильтр `SPORT_TYPES_RUN = {100, 101}` в клиенте Coros молча отбрасывал её, а лог писал
«API вернул пустой список» (API отдаёт полный список, фильтр — клиентский). Кнопка синка здоровья на веб-форме падала с
`TypeError: sync_health_for_user() got an unexpected keyword argument 'pending'` (со Sprint 18, 14.07.2026; авто-синк и
Telegram `/sync` этим путём не идут — не замечали).

### Fixed
- **Беговые режимы Coros — все четыре**: `SPORT_TYPES_RUN = {100 Run, 101 Indoor Run, 102 Trail Run, 103 Track Run}`
  (`src/watch/coros.py`; 101 раньше был ошибочно назван trail). `download_activity` уже передаёт `sportType` активности.
- **`run_sync_for_user` передаёт `pending` только activity-синку** (`orchestrator.py`) — кнопка «здоровье» на веб-форме работает.
- **Окно `since` списка активностей — `ACTIVITY_SYNC_LOOKBACK_DAYS` = 7 дней, не 2 часа** (`constants.py`, `activities.py`):
  таймстемп синка двигался и на пустом результате, поэтому тренировка, выгруженная часами в облако позже 2 ч после старта
  (или отброшенная фильтром до фикса), терялась навсегда. API дат не принимает (полный список постранично), дедуп — по
  `external_activity_id` по всей истории — широкое окно безопасно. Именно оно забирает пропущенную тренировку 17.09.

### Changed
- **Диагностика фильтра**: `list_activities` пишет `Coros list_activities: raw=N run=M dropped_sport={код: n} dropped_since=K
  newest=… since=…`; строка «API вернул пустой список» в `activities.py` переформулирована — «после фильтра новых нет».

### Tests
- `tests/test_sync_filters.py`: режимы 100–103 проходят, 200/999 нет; строгий `since`; лог диагностики; `run_sync_for_user`
  health без `pending` / activity с `pending`; окно since — дни, не часы.

### Backlog
- #357: в контейнере `app` uvicorn отключает именованные логгеры, созданные при импорте, `fix_logger_after_uvicorn`
  восстанавливает только три — строка диагностики `watch.coros` до лога не дошла; клиент Coros переведён на логгер `app`.
- #356: `bin/backfill_raw_fits.py` хардкодит `sportType=100` при скачивании FIT — для сессий 102/103 упадёт (тип спорта в
  `TrainingSession` не хранится).

## [17.09.2026] — «Интенсив — не раньше …» не уезжает по правилам 11/18/19 и не читается как обещание (#306 ч.2, #355)

Инцидент 16–17.09 (прод): план недели 14–20.09 без качественных дней (Вт/Чт/Пт лёгкие, Вс длительная, Сб — отдых), но
утро Ср сказало «интенсив вернётся не раньше пятницы», утро Чт — «интенсив всё так же ждёт нас с субботы», карточка —
«Интенсив — не раньше 19.09 09:30». Источник — `earliest_next_hard`: пробежка Вт 15.09 получила флаги
`poor_interval_recovery` и `downhill_load_high`, а правила 11/18/19 считали срок как «сейчас + 48/48/24 ч», пока флаг в
окне поиска (4/3/2 дня) — дата ехала пт → сб → вс → пн, а LLM читал границу как план.

### Fixed
- **Правила 11/18/19 заякорены к флагнутой тренировке** (`state.py` → сигналы `poor_interval_recovery_at` /
  `quality_volume_exceeded_at` / `downhill_load_at` из `InsightRepository.recent_flag_sessions`; `p1_safety._anchored`):
  срок = начало тренировки + часы правила, между ходами не меняется, вышел → правило молчит целиком (у 19 — и без
  `max_zone=3`). Без якоря (снимки/тесты) — прежний `now + часы`. Симметрично правилу 10 (#306, 07.09). Для инцидента: срок
  правила 11 — Чт 17.09 12:23, правило 19 к утру Чт уже погасло.
- **Промпт** (`SAFETY_CONTRACT`): `earliest_next_hard` — граница, не план и не обещание; качественный день есть только в
  `planned_workouts` (tempo/interval/race), иначе неделя лёгкая — так и говорить; когда вернётся качество — решает план недели.

### Changed
- **Строка «Интенсив — не раньше …» на нехардовой карточке — только при качественном дне в действующем плане недели**
  (решение владельца 17.09.2026): `planning_rows.hard_day_planned` (с сегодня по вс, последняя действующая строка на дату,
  `superseded` не считается) → `render_prescription(hard_planned=)` / `_show_earliest`; порог остатка ≥ 60 мин (#306)
  сохранён; на tempo/interval/race — всегда. Все три вызова в `chat_flow` (утро, чат, детерминированное утро) передают флаг.
- Тесты: `tests/coach/test_earliest_hard.py` (+6: якорь трёх правил, гашение по сроку без кэпа зоны, legacy без якоря,
  сигналы `*_at` из разбора, `hard_day_planned`, карточка без плана), два теста пояса в `test_render.py` — `hard_planned=True`.
  Всего 1135.

### Docs
- CLAUDE.md (Safety по содержимому, точка возобновления); BACKLOG: #355 закрыт сразу — в архиве.

## [17.09.2026] — Допуск объёма по состоянию и диапазон времени в назначении дня (решения владельца)

Инцидент 17.09 (прод): утро урезало плановый лёгкий день 40 → 30 мин («⚠️ Объём урезан … осталось 18.2 км, из них
15.7 уже назначено») при чистом вердикте усталости — три правила распределения, ни одного стойкого. Разбор: 1.4 км
лёгкого — шум маршрута, «10 %» — потолок, не норма; причина превышения — лишний беговой день в пн + длиннее вт, но
инструмент грубый: режет плановый день ради ±5 %, которых у модели нет. Разбор — `TASK_2026-09-13_athlete_requests.md` §9.

### Changed
- **Допуск объёма дня — по состоянию** (`day_caps.day_targets` → `day_volume_tolerance_pct`): нет стойких сигналов
  усталости/здоровья (`planning_safety.fatigue_signals` = `triggered` − `VOLUME_TRANSIENT_SAFETY_RULES`) → 15 %
  (`DAY_VOLUME_TOLERANCE_RELAXED_PCT`), есть — строго 5 % как раньше; `/plan` (`cap_week_volume`) — 5 %. **Гистерезис**
  (`DAY_CAP_MIN_CUT_MIN` 10 / `DAY_CAP_MIN_CUT_KM` 1.0): срез короче — не делаем при любом допуске (шум, не перегруз;
  вместо `LONG_RUN_CAP_TOLERANCE_KM` в `cap_day_volume`). Заметка называет допуск и причину («допуск 5 % — усталость
  (hrv_low)»); `context_block.unallocated_km` и `SAFETY_CONTRACT` — по допуску дня. 17.09 с новым правилом: 40 мин без
  урезания; 13.09 (50 → 72 мин/10 км) по-прежнему режется `cap_long_run` до 58 — потолок доли недели строгий.
- `planning_safety.volume_hold` переписан через `fatigue_signals` (семантика прежняя, список правил один).

### Added
- **Диапазон времени ровного дня** — `coach/volume_range.py` (pure): для easy/recovery/long без сегментов и без темпа
  низ = max(30, 75 % верха; long — 85 % и не ниже порога длительной), производный ключ `volume.duration_min_low`,
  считается один раз в `prescriber.finalize` (все пути — чат, утро, план, двухпроходные кэпы — идут через него; низ
  всегда от итогового верха). Верх `duration_min` остаётся единственным жёстким числом (суммы, кэпы, экспорт,
  `rehydrate`, дедуп утра — без правок; миграции нет). Карточки: «30–40 мин · пульс до 138 · ~7:03/км → ≈4.3–5.7 км»
  (`render._hr_lead_lines`, `render_prescription_short`, `render_week` строка дня / `_change_label` / пропуск); старые
  строки без низа — точка. `numeric_check`: эталон минут/км — интервалы, «35 мин» при «30–40» не режется.
  `plan_vs_actual`: `duration_min_low` в `planned`/`baseline`, `within_range`, факт внутри диапазона → `volume_ratio`
  1.0 (`volume_ratio_raw` — сырое), флаг перебора без изменений; `INSIGHTS_SCHEMA_VERSION` 13. `week_plan_review`
  сравнивает верх/дистанцию, не `volume_json` целиком (иначе строки до/после релиза — ложный `changed_in_chat`).
  `OUTPUT_CONTRACT`: LLM диапазон не задаёт и не проговаривает.
- Тесты: `tests/coach/test_volume_range.py` (11), правка `test_predicted.py`; всего 1127.

### Docs
- TASK_2026-09-13 §9; BACKLOG: #352/#353 закрыты сразу (архив), #354 добавлен (`residual_load`/`planned_min` от верха,
  Coros-экспорт диапазона); METRICS_GUIDE §5; ARCHITECTURE; CLAUDE.md; README.

## [17.09.2026] — Потолок недельного объёма и календарь целевых стартов (#243 ч.1, решения владельца)

Вопрос владельца: «объём растёт от недели к неделе — до какого?» Потолка не было: +10 %/нед три недели, разгрузка
75 %, новый цикл от последней недели роста — ~+33 % за 4 недели без границы; будущей даты старта в системе не
существовало. Литература в базе (гайды 41/45/47/60/61, черновики): абсолютных км/нед нет, синтез для любителя на
10 км/ПМ — пик 45–60 км, коридор 80–100 % пика, крыша 70; подготовка по гайду 60 (10 км 12 нед, ПМ 15, марафон 18;
тейпер за 2 нед 70–80 %, неделя старта 50–60 %). Свод и решения — `docs/coach/TASK_2026-09-17_races_and_volume_ceiling.md`.

### Added
- `coach/config.py`: потолки по дистанции цели `RACE_VOLUME_CEILINGS_KM` (5 км 40 · 10 км 50 · ПМ 55 · марафон 65),
  крыша `RACE_VOLUME_HARD_CAP_KM` 70, без стартов `RACE_VOLUME_DEFAULT_CEILING_KM` 55; тейпер `RACE_TAPER_VOLUME_PCT`
  0.75 / `RACE_WEEK_VOLUME_PCT` 0.55 (= key_rules гайда 60), горизонт 18 нед, `RACE_RATIONALE`.
- `coach/races.py` — календарь стартов в `params_json["races"]` (без миграции): LLM сообщает факт
  `CoachTurn.races[RaceReport{add|cancel, date «ГГГГ-ММ-ДД»|«ММ-ДД», days_ahead, distance_km, label}]`, код
  подбирает год (ближайшее будущее), валидирует (прошлое / дальше 52 нед — отказ текстом), пишет, отвечает
  строкой «Записал старт: 15.11 · Осенний ПМ (21.1 км) — до него 8 нед, ориентир пика ~55 км/нед»; несколько стартов,
  дубль по дате обновляет, отмена — однозначный кандидат иначе перечисление, прошедшие гаснут `done` при записи;
  блок `races (params)` в today-контексте (чат и `/plan`-путь через `weekly_plan_turn.apply_turn_facts`).
- `coach/race_plan.py` — периодизация (pure): `goal_for_week` — потолок = max по стартам в горизонте, фазы
  `race_week` (объём max(55 % пика, дистанция), качества нет) / `taper` (75 %, ≤ 1 качественный) / `build`
  («достижимый пик» `min(потолок, prev × 1.1^growth_weeks)`, `peak_capped_by_date` — честно в шапке) / `base` /
  `maintenance` (упёрлись в потолок — плато, прогресс в качество); `place_race` — старт кодом на день гонки (как тест
  ПАНО), качество в неделю старта → лёгкие, накануне ≤ 30 мин; `header_suffix` — «до старта N нед (…)», «тейпер ·
  старт …», «неделя старта (…)», «объём на потолке ~55 км — прогресс в качество».
- `planning.week_targets`: `goal`/`capped_by_ceiling`/`race_day_ahead`; `target_km = min(прогрессия, потолок фазы)`,
  `hard_days_max ≤ hard_days_cap`, тест ПАНО не в тейпере/неделе старта; после недели старта мезоцикл с 1.
  `advance_mesocycle`: неделя на потолке — «рост» для счётчика (разгрузка на плато придёт), тейпер/неделя старта —
  не база цикла; мета несёт `goal_phase`/`goal` для `/week`. `weekly_plan`: `place_race` до кэпа частоты,
  `races.mark_race` — маркер `target_json["race"]` и на понижённый safety тип («🏁 Старт · в лёгком режиме (safety)» —
  #243 п.4), заметки 🏁/⚠️. `day_caps`: `race` кэпами объёма и частоты не режется. Промпты: строка персоны про
  старты (здоровье важнее рекорда, прогресс относительно себя), абзац в `SAFETY_CONTRACT`, правило `races` в
  `OUTPUT_CONTRACT`, фазы в `PLAN_PROMPT`; запросы гайдов 60/48 в тейпере/неделе старта.
- Тесты: `test_races.py` (11), `test_race_plan_e2e.py` (2), `test_planning.py` (+4), `test_day_caps.py` (+2),
  `test_week_view.py` (+1), `test_coach_config.py` (+1); всего 1116.

### Fixed
- `planning.advance_mesocycle` стирал окно доступности (`week_plan.availability`, `set_availability`) каждым
  `/plan` — мета собиралась заново без ключа (BACKLOG #351, найдено при разборе; на проде окно было пустым).

### Changed
- Выносы по лимиту ~400 строк: `coach/weekly_plan_turn.py` (`apply_turn_facts`, прежнее имя реэкспортируется),
  `coach/llm/prompts_plan.py` (`PLAN_PROMPT`, реэкспорт из `prompts`).

### Docs
- `docs/coach/TASK_2026-09-17_races_and_volume_ceiling.md` (свод литературы, решения, дизайн, остаток #243); BACKLOG: #243 → 🟡 (часть 1), #350 (скорость роста
  10 %/нед vs Дэниелс/Фицджеральд), #351 → архив; DEV_PLAN §4/§9; ARCHITECTURE; README; гайд 20 — примечание об
  источнике правила 10 %; CLAUDE.md.

## [17.09.2026] — Частота беговых дней в чате/утре: мягкий кэп лишнего дня и правило 22 (решения владельца)

Вопрос владельца: оценит ли тренер просьбу поставить тренировку в день, назначенный отдыхом. По коду: объём и
интенсивность — да (#338–#343, safety), частота — нет: плановый отдых строкой не хранится (`weekly_plan._clean_days`,
гвард `blocked_by_unavailable` видит только отменённые подопечным дни), `run_days_max`/`rest_days_min` держал только
`/plan` (`enforce_run_days`) и в контекст чата они не попадали, а серию беговых дней без выходного safety не видел
(`consecutive_hard_days` — только качественные). Решения владельца 17.09.2026: кэп **мягкий** (понижение до лёгкой
≤ 30 мин с пометкой, не отказ) + **правило 22** в safety. Разбор — `docs/coach/TASK_2026-09-13_athlete_requests.md` §8.

### Added
- `coach/day_caps.py`: `cap_run_days` — беговые дни недели исчерпаны (факт ∪ действующий план ≥ `run_days_max`;
  множество дат без двойного счёта — `run_days_used`, `week_done`/`week_targets` отдают `done_dates`) и день не из
  плана недели (`PLAN_STATUSES`; строка `proposed` из чата день не освобождает — повтор просьбы кэп не обходит) →
  `easy`, зона ≤ 2, без структуры и темпа, не дольше `PLAN_EASY_MIN_MINUTES`; заметка «⚠️ Беговые дни недели
  исчерпаны (N из M) — вместо … оставил лёгкие 30 мин». Первый кэп в `finalize_with_caps` (чат и утро). Плановые
  дни — каркас недели, их защищает `enforce_run_days`, кэп частоты их не трогает.
- Контекст модели: `run_days_max`, `rest_days_min`, `done_runs`, `run_days_used` и `frequency_rule` в
  `week_targets (planning)`; абзац о частоте в `SAFETY_CONTRACT`; `PLAN_PROMPT` — качественный день не на
  (`SAFETY_RUN_STREAK_MAX_DAYS`+1)-й беговой день подряд (иначе утро срежет его правилом 22).
- Правило 22 `run_streak` (`rules/p1_safety.py`): `consecutive_run_days ≥ SAFETY_RUN_STREAK_MAX_DAYS` (3; гайд 61
  «без 3 тренировок подряд», гайд 47 «день отдыха в неделе — всегда») → `max_zone 2`, без `HARD_TYPES`, тренировка
  разрешена. Сигнал — `state._week_signals` по локальным датам назад от сегодня (сегодня без пробежки — от вчера);
  `planning_safety.project_state` обнуляет его для дней плана > 0 (частоту будущих дней держит `enforce_run_days`);
  входит в `VOLUME_TRANSIENT_SAFETY_RULES` (объём недели не держит).
- Тесты: `test_day_caps.py` (+8: чистые `run_days_used`/`cap_run_days`; e2e «хочу сегодня побегать» в день без
  строки → 30 мин + заметка, повтор просьбы не обходит кэп, свободный день проходит без заметки, плановый день не
  трогается), `test_safety_week_rules.py` (+3, включая проекцию), `test_state.py` (+1 серия), `test_planning.py`
  (`week_done.dates`), `test_coach_config.py` (порог, `run_streak` в разовых); всего 1096.

### Changed
- `day_caps.planned_km_other_days` → общий запрос `_other_day_rows` + `_rows_km`: один запрос строк недели на ход
  для объёма и частоты.

### Docs
- BACKLOG: #347/#348 закрыты сразу (архив), #349 добавлен (контекст чата показывает `hard_days_max=0` на всю неделю
  при разовом правиле); DEV_PLAN §4 таблица правил (22) и §9; ARCHITECTURE; METRICS_GUIDE §11; TASK_2026-09-13 §8;
  README; CLAUDE.md (пункт про частоту, точка возобновления).

## [16.09.2026] — Объём недели: плоский только по стойким сигналам, мезоцикл считает недели роста (решения владельца)

Вопрос владельца: три недели подряд ~26 км при полном выполнении плана. Разбор по проду: 31.08 — отмены
подопечного; 07.09 — правило 17 при прежней политике «всегда плоский» (отменена 12.09); 14.09 — одна короткая
ночь (`sleep_short`) вечером планирования заморозила объём недели; 21.09 по счётчику пришла бы разгрузка до
~20 км без единой недели роста.

### Changed
- `coach/config.py`: `VOLUME_TRANSIENT_SAFETY_RULES` = `INTENSITY_ONLY_SAFETY_RULES` + `sleep_short`,
  `sleep_very_short`, `hard_streak` — разовые сигналы дня закрывают интенсив, но объём недели не держат;
  `planning_safety.volume_hold` держит объём плоским только по стойким сигналам (HRV, recovery %, ACWR/ATI,
  боль, болезнь, detraining, монотонность, нет данных) или при пустом списке.
- `planning.week_targets` / `advance_mesocycle`: мета `week_plan.grew` (неделя запланирована как рост после
  согласования с safety); счётчик мезоцикла двигается только после недели роста или разгрузки — плоская неделя
  повторяет свой номер, deload наступает после трёх фактических недель роста. Старая мета без `grew` = «не росли»:
  на проде 21.09 будет «неделя 3/4 (рост) · цель ~29 км», не разгрузка.
- Тесты: `test_flat_week_does_not_advance_mesocycle`, `test_advance_mesocycle_writes_grew`, репродукция вердикта
  13.09 в `test_apply_safety_volume_hold_only_for_fatigue_rules`, гвард списка в `test_coach_config`; всего 1083.
...
### Fixed
- **Загрузка верила расширению файла** — новый `parsers/detect.py::sniff_kind`: FIT по сигнатуре `.FIT` в заголовке,
  TCX по XML-началу (с учётом BOM); несоответствие расширению → файл в сводку ошибок, в `raw/` не сохраняется и не
  парсится (#78).
- **Аудит `/sync` бота писал накопленный итог по всем брендам в событие каждого бренда** — теперь `found`/`processed`
  по бренду плюс `health` (#114).
- **`/logs` красил строку по подстроке** — слово WARNING/ERROR в тексте INFO-сообщения меняло класс; уровень берётся из
  поля формата логгера, CRITICAL → ERROR, строка без поля → INFO (#120).
- **FIT: высота 0 м считалась пропуском** (`enhanced_altitude or altitude`) — `_pick_altitude` различает None и 0 (#106).
- **Аудит настроек писал ложное изменение `watch_email` при сохранении без бренда часов** ('' против None) — diff только
  при выбранном бренде, нормализованно (#127).
- **«Сегодняшних тренировок» в `/sync` считалось от полуночи серверного пояса** — теперь `user_now(user)` (#124).
  Тесты: `test_detect.py` (новый), `test_logs_route.py`, `test_settings_route.py`, `test_auto_sync.py` (+1 каждый).
  `BACKLOG.md`: шесть строк — в архив (274 закрытых), перечисления P3 подчищены.

## [11.09.2026] — BACKLOG-гигиена: шесть устаревших строк аудита 07.2026 закрыты

### Changed
- Сверены с кодом и закрыты как невоспроизводимые/уже сделанные: #103 (двойной `save_dashboard_data` — ветки не
  пересекаются), #104 (пустой StartTime в TCX защищён), #112 (хардкода 15.0 нет), #117 (catch-all веса заменён роутером
  текста), #123 (о сгенерированном пароле пользователю сообщается), #81 (rate-limit на upload/settings есть, `/logs` —
  только для вошедшего). Причины — в `docs/archive/BACKLOG_closed.md` (268 закрытых); перечисления P3 подчищены.

## [11.09.2026] — Гвард «181 → 181» в адаптивном max_hr, валидация max_hr в /settings (#333, #239)

### Fixed
- **`hr_max`: принудительное обновление на то же значение** — при пиках 185/183/181 и профиле 181 третий дневной пик
  равнялся профилю, уходили аудит-событие и уведомление «181 → 181». Теперь `new_value <= profile` → предупреждение
  с кнопкой (пик батча всё же выше профиля), без аудита/обновления; путь обновления вынесен в `_force_update` (#333).
- **`POST /settings` принимал любой `max_hr`** — теперь диапазон `MAX_HR_MIN..MAX_HR_CAP` (100–220, как у кнопки бота;
  `MAX_HR_MIN` переехала из хендлера в `config/constants.py`): вне диапазона — редирект `/settings?error=max_hr`,
  на странице строка ошибки, значение не сохраняется; границы `<input>` — из констант (было `max='250'`) (#239).
  Тесты: `test_hr_max.py` (+1), новый `tests/test_settings_route.py` (сессия роута привязана к соединению фикстуры —
  in-memory SQLite не делит таблицы между потоками TestClient). `BACKLOG.md`: #333/#239 — в архив (262 закрытых).
...
### Added
- **После принудительного обновления max_hr тренировки текущего батча пересчитываются** (решение владельца
  11.09.2026, пересмотр «в v1 нет» от 06.08): раньше зоны, тип и insights батча оставались посчитанными по старому
  максимуму, и коуч разбирал их по ним. `hr_max.reanalyze_batch_after_raise(db, user_id, session_ids, old, new)` —
  цикл `reanalyze_training(..., check_max_hr=False)` (новый kw-флаг: повторная оценка пика выключена, иначе тот же
  батч оценивался бы снова), каждая тренировка изолирована try/except (сбой не роняет синк/загрузку), одно
  уведомление «↻ Пересчитал N тренировок … по новому максимуму». Вызовы: `sync/activities.py` — синхронно,
  ДО старта треда разбора коучем (тот пишет те же строки insights — без гонки сессий); `web/routes/uploads.py` —
  в запросе, по `saved_ids` батча. Ручная смена max_hr по кнопке/в настройках — без изменений.
  Тесты: `test_hr_max.py` (+4: флаг, изоляция сбоя, тишина без пересчёта, склонение).
  `BACKLOG.md`: #237 — в архив (256 закрытых); новый **#333** — латентный «181 → 181» в `_evaluate_max_hr_raise`,
  когда N-й дневной пик равен профилю (не чинили «заодно»). Следующий номер — #334.

## [11.09.2026] — Numeric checker v2: предложение прозы с чужим числом вырезается (#247)

### Added
- **`coach/numeric_check.py` v2 (решение владельца 11.09.2026)**: `mismatch_pairs` отдаёт сырые токены расхождений
  (`check_prose` — прежний список описаний поверх него), `trim_mismatched_prose` вырезает из прозы предложения
  с такими токенами (абзацы сохраняются; пустой остаток → `NUMERIC_TRIM_FALLBACK_TEXT`), добавляя один раз хвост
  `NUMERIC_TRIM_NOTE` «Числа — в карточке ниже». `chat_flow._llm_chat_turn`: проза — префикс текста ответа, поэтому
  урезание точечное, карточка/хвосты (отмены дней, болезнь, GPS-предупреждение) нетронуты; `meta.prose` хранит
  урезанную прозу (#258), `meta.numeric_trimmed` — удалённые предложения, `meta.numeric_mismatch` — как в v1.
  Тот же путь для недельного отчёта (`prose_numbers`: чисел в прозе быть не должно — числа в карточке «Итоги недели»).
  Выключатель `COACH_NUMERIC_TRIM_PROSE` (`llm/config.py`; False → только детект, как v1).
  **Основание**: за 29.08–11.09 на проде 0 расхождений из 83 ответов коуча (17 morning с карточкой, 44 chat,
  10 review, 9 plan, 3 weekly) — ложных срабатываний тоже нет, страховка бесплатна. Тесты: `test_numeric_check.py`
  (+4: токены, trim, fallback, e2e без изменений при совпадении), `test_review_weekly.py` (weekly-проза режется).
  Доки: `docs/coach/ARCHITECTURE.md`, `DEV_PLAN.md` §11.5, `CLAUDE.md`. `BACKLOG.md`: #247 — в архив (255 закрытых).

## [11.09.2026] — Карточка недели: прошедший день отдыха без «✗ пропущен» (#331)

### Fixed
- **`/week`: прошедший день отдыха рендерился как «✗ Вт 08.09 — 🛌 Отдых · пропущен»** — `render_week._fact_line`
  любой прошедший день без связанной тренировки считал пропуском. Теперь rest в прошлом — нейтральная строка
  «🛌 Отдых» без ✓/✗ и минут; отдых по отмене подопечным (маркер `UNAVAILABLE_RATIONALE`) подписан
  «по твоей просьбе», как у будущих дней; легенда «✓ факт · ✗ пропущен» появляется только при беговых прошедших
  днях. Тест `test_render_week_plan_past_rest_day_is_neutral`. `BACKLOG.md`: #331 — в архив (254 закрытых).
...
### Fixed
- **Два теста падали на чистом дереве в зависимости от даты.** `test_illness.py::test_week_targets_exclude_paused_days`
  брал реальное «сейчас»: среди недели `/plan` = остаток недели (#293), и в пт/сб дат остатка меньше паузы после
  болезни — теперь якорь «будущий понедельник 09:00» (как `_sunday`/`_wednesday` в тестах плана), окно полной
  недели всегда ≥ паузы (#328). `test_weekly_plan.py::test_weekly_plan_keeps_strides_segments` сломал #317 (07.09):
  «длительная» 50 мин короче `LONG_RUN_MIN_MINUTES` перемаркируется в easy, строк easy стало две и `.one()` в тесте
  падал `MultipleResultsFound` — длительная в сценарии 70 мин, строка ускорений выбирается по дате дня 2, тип
  проверяется отдельно (#332). Код не менялся. `BACKLOG.md`: #328/#332 — в архив (250 закрытых), следующий номер — #333.
...
### Changed
- **Датчик температуры часов под одеждой** (вопрос владельца: зимой часы покажут +5 при −10 на улице).
  Показание часов в данные не пишется (`avg_temperature` и сегменты — только Open-Meteo), оно лишь
  фолбэк `heat_block` при отказе погоды (2 из 44 тренировок лета); ниже +10 °C сдвиг пульса зажат,
  так что «+5 при −10» безвреден. Опасен фолбэк +15…+25 под куркой → ожидание уезжает на 3–4 уд/мин
  и ложный `heat_flag`. Прод (42 пары часы/погода): расхождение растёт с холодом — 0…2 °C при
  +25…30, 5…7 при +10…15, константа 3.3 неверна на краях. Сделано: `services/watch_temp_bias.
  recent_watch_bias` — медиана (часы − погода) последних 5…10 тренировок за 45 дн
  (`WATCH_BIAS_*`), `heat_block(watch_bias_c, recent_weather_c)`: поправка адаптивная (`bias_source`
  recent|default), гвард ложной жары (`heat_flag` по часам только если недавняя погода ≥ порога −
  `WATCH_HEAT_CONFIRM_MARGIN_C` 5); в блок пишутся `watch_temp_c` и `watch_minus_weather_c` — данные
  для зимнего исследования #301.
- **Путь «холод»**: `heat.cold_flag` при t ≤ `COLD_TEMP_THRESHOLD_C` 0 — только по погоде → контекст-
  флаг `cold` в `computed.flags` (как `heat`, в enum assessment не входит), `review_guides_queries`
  добавляет запрос «мороз ветер снег гололёд покрытие» → гайд 49 Швеца (ветер опаснее мороза,
  глубокий снег — не день объёма, гололёд — короче шаг; два правила добавлены в `key_rules`, бюджет
  дайджеста 60 → 64 строки); REVIEW_PROMPT: ниже +10 °C сдвиг не экстраполируется, на снегу/гололёде
  усилие оценивать по пульсу и ощущениям, темп вторичен. Формула сдвига пульса в мороз намеренно
  не расширена — нет ни одной пробежки при ≤ 0 °C (#301, после 31.12.2026).
- `INSIGHTS_SCHEMA_VERSION` 9 → 10 — история пересчитывается лениво.

### Fixed
- `parsers/weather.py`: метки Open-Meteo (`timezone=UTC`, naive ISO) парсятся как UTC (`_epoch`) —
  раньше `fromisoformat(t).timestamp()` трактовал их в поясе хоста (в контейнере UTC не проявлялось);
  URL — из `WEATHER_API_URL` (golden rule №1), мёртвая `WEATHER_CACHE_TTL_SECONDS` удалена.
- Тесты: `test_effort.py` (адаптивная поправка, гвард, `cold_flag` только от погоды),
  `test_workout_insights.py` (поправка из истории пользователя, `watch_minus_weather_c`, `cold` в флагах),
  `test_weather_avg.py` (aware-даты, UTC), `test_guide_queries.py`, `test_session_metrics.py`.
  BACKLOG: #301 дополнен, новые #325 (полосы темпа не знают о сезоне), #326 (день запроса погоды).
...
### Changed
- **#275** — `gps_quality.watch_stride_m`: длина шага из FIT session (`avg_step_length_mm`, диапазон
  здравого смысла 0.5–1.6 м) передаётся в `process_trackpoints(watch_stride_m=)` из `fit_parser` и
  `reanalyze`; когда калибровка по чистым окнам невозможна, оценка дистанции по шагам берёт шаг
  часов (`quality="watch"`) вместо дефолтного 1.0 м (`"rough"`). Проверка на проде: 0.89–0.91 м;
  битый файл 01.09 (1.64 м) отсекается диапазоном.
- **#316** — `numeric_check.check_plan_prose`: для полной недели сверяет прозу плана с карточками —
  число беговых («4 пробежки») и лёгких («пять спокойных дней») дней, упоминание темповой/
  интервалов/длительной без такой карточки; расхождения — в лог и `meta.prose_type_mismatch`
  assistant-строки, текст не режем (как #247 v1).
...
### Fixed
- **#313** — `sync/utils.classify_watch_error`: 401/403 и отказ входа → «учётные данные»; 5xx, сеть,
  таймаут → «сервер часов недоступен, повторим автоматически»; иначе — прежний текст. Причина
  фиксируется в `_make_client` транзитно на cred и попадает в алерт «не работает» и в ответ `/sync`
  (06.09 Coros лежал с 504, алерт советовал проверить пароль).
- **#312** — `orchestrator.record_manual_sync_result` — общий итог ручного синка для web и
  Telegram: успех двигает `last_*_sync_at`, сбрасывает счётчик сбоев и шлёт «✅ восстановлена»,
  если алерт уходил; сбой — счётчик не трогаем, сбрасываем кэш токена. Раньше `/sync` из бота
  ничего из этого не делал, и алерт закрывался только следующим авто-синком.
- Тесты `tests/test_auto_sync.py` (+4).

## [07.09.2026] — GAP на спусках: пол фактора 0.88 (#298), insights v8

### Changed
- `analysis/gap.gap_factor`: на спусках не ниже `GAP_FACTOR_MIN` (0.88, `config/constants.py`) —
  полином Minetti занижал усилие на спуске (−5 % → 0.76), остаточный эффект уклона поверх GAP был
  положительным (исследование 02.09). Подъёмы и плоскость не тронуты. Проверка на 4 самых
  холмистых тренировках прода за 90 дней: километровый GAP не изменился (уклоны км < 2 %),
  посэмпловый средний фактор +0.3–0.9 % (дрейф/downhill-блок).
- `INSIGHTS_SCHEMA_VERSION` 7 → 8: computed_json истории пересчитывается лениво при обращении
  (`get_or_compute`) — GAP-пол и `heat.temp_source` появятся у старых тренировок единообразно.
- METRICS_GUIDE §4 (строка GAP), тест `test_gap_factor_downhill_floor`.
...
### Changed
- Гайд `20_progression`: разгрузочная неделя 75 % объёма (было 60 %) — как гайд 60 и
  `config.CYCLE_3_1.deload_volume_pct`; LLM больше не видит два разных числа в дайджесте.

## [07.09.2026] — «Интенсив — не раньше …» не дрейфует и не шумит на лёгких (#306)

### Fixed
- Срок восстановления считается от начала последней тренировки плюс часы по типу
  (`skills/recovery.ready_at`, сигнал `recovery_ready_at`), правило 10 safety берёт его напрямую —
  раньше «сейчас + округлённый остаток» плыл с каждым ходом (03.09: 10:16 → 10:18 через 38 мин).
  Без сигнала — прежний путь через `recovery_hours_left` (совместимость тестов/снимков).
- На карточке нехардового дня строка показывается только при остатке ≥ `EARLIEST_HARD_HIDE_MIN`
  (60 мин, `coach/config.py`); на качественном дне — всегда (`render._show_earliest`).
- Тесты `tests/coach/test_earliest_hard.py`; два теста рендера пояса переведены на будущий якорь.

## [07.09.2026] — Правило 16 в плане недели: окно доли Z3+ сдвигается по дню (#315)

Решение владельца: 7-дневные окна правил 16/17 в плане недели сдвигаются по дню плана, а не
закрывают интенсив на всю неделю по сегодняшнему вердикту (правило 17 прогнозировалось с утра).

### Changed
- `planning_safety.hard_share_by_day` — доля времени Z3+ «за 7 дней» на каждый день вперёд:
  тренировки старше окна от этого дня выходят, будущие не считаются (консервативно); меньше
  `HARD_SHARE_MIN_MINUTES_7D` в окне → None. `project_state`/`quality_reopens_at` принимают
  `hard_shares`; `weekly_plan` передаёт их в финализацию каждого дня и в прогноз открытия качества.
- `TrainingRepository.zone_minutes_by_session` — минуты по зонам на тренировку (посекундные из
  insights, иначе сегментное приближение); `zone_distribution` — агрегат поверх неё.
  `HARD_SHARE_LOOKBACK_DAYS = 7` вместо литерала в `state.py`.
...
### Fixed
- **#319** — `/plan` среди недели после `PLAN_TODAY_CUTOFF_HOUR` (20:00 локально) не назначает
  тренировку на уходящий день: `plan_window(today, trained_today, hour)`, `week_targets(now=…)`
  получает локальное время из `generate_weekly_plan` (07.09 карточка в 18:00 ставила «▶ Пн»).
- **#320/#314** — тесты не зависят от дня недели запуска: фикстура `plan_athlete` кладёт пробежки
  на прошлую полную локальную неделю (пн/ср/пт/сб) вместо `i*2` дней назад; сверка «план vs факт»
  проверяет прошлую неделю; detraining-тест — ровно 14 дней; `test_week_view` не совмещает
  «свежую строку» с сегодняшним днём; убрана ссылка на файл вне репозитория. Полный прогон в
  понедельник — зелёный (943 passed).

## [07.09.2026] — База знаний: гайды по книге Швеца «Я бегу марафон» (E2.1, #245)

Из двух новых книг в `books/` методику содержит одна: Лаймон «Марафон» — художественный рассказ
(отклонён). Швец (1983, оздоровительный бег) разобран целиком; взяты темы, которых не было
в гайдах Лидьярда/Фицджеральда/Дэниелса, устаревшие советы помечены «не применять».

### Added
- Гайды `src/coach/knowledge/guides/47–50` (Швец): вход в бег и возврат после паузы через
  ходьбу→бег; день гонки и марафонская раскладка; условия (погода, покрытие, обувь) и
  самоконтроль без приборов; болезнь и вынужденные паузы (сроки перерыва, температура, застолье).
- `knowledge/loader.plan_guides_queries(targets)` — запросы к базе знаний для плана недели:
  при `detraining_return` — гайды 47 + 61 вместо общей прогрессии мезоцикла.
- `tests/coach/test_guide_queries.py` — новые гайды находятся поиском, боль → колено не сдвинулась,
  дайджест key_rules ≤ 60 строк, жара → третий запрос, план при возврате после паузы.

### Changed
- `review_guides_queries`: тип `race` → «соревнование гонка раскладка» (гайд 48); флаг жары
  (`computed.heat.heat_flag`) добавляет запрос «жара погода условия» третьим (гайд 49).
- `turn_context.build_extras(guides_query=…)` принимает строку или список запросов.
- Персона (`llm/prompts.py`) перечисляет Швеца среди источников методики.
- DEV_PLAN E2.1; ориентир размера дайджеста ≤ 60 строк (было ≤ 50, гварда не было).
...
### Changed
- **Конкретика структурного дня (запрос владельца 06.09).** Компактная строка недели/короткой карточки
  включает отдых между отрезками: «5×20 сек свободно (отдых 2 мин трусцой)», «6×90 сек до 167 (отдых до
  пульса ≤125)», ускорение без чисел — «(отдых до полного восстановления)» (`compact_segments`,
  `_compact_recovery`); уточнение решения 02.09 «структура одной строкой». Длительность структурного
  дня задаёт код из суммы сегментов (`finalize`: 39 → 42 мин, rationale «сумма сегментов», predicted
  пересчитан) — строка недели больше не спорит с карточкой дня. Карточка дня с ускорениями печатает
  «как выполнять» (`STRIDE_HOWTO`, гайды 46/61, без чисел). `PLAN_PROMPT`: у ускорений обязателен
  `recovery.duration_min` 1–2, длительность элемента = сумме сегментов.

### Fixed
- **Ускорения — по усилию, не по пульсу.** Карточка показывала «5×20 сек до 138»: per-segment clamp
  и потолок пульса применялись к 15–20-секундным ускорениям, где пульс не успевает (гайды 45/46).
  `enrich_and_clamp_segments` помечает `is_stride`-сегменты (`stride: True`): зона не клэмпится,
  `hr_ceiling`/темп не считаются, усилие по умолчанию `STRIDE_DEFAULT_EFFORT` («свободно»);
  `compact_segments`/`render_segment_lines` печатают усилие («5×20 сек свободно»), отдых без чисел —
  «до полного восстановления». Исключение из решения владельца 02.09 «пульс в уд/мин».
- **Структурные дни не ужимаются.** `cap_week_volume` масштабировал длительность дня с сегментами
  (39 мин при сегментах на 42) — теперь дни с `segments` фиксированы, ужимаются только ровные лёгкие;
  структурная длительная выше потолка — предупреждение «проверь вручную», не молчаливый срез.
- **След урезания в `proposal_json`.** Урезанные копии получают `rationale` «урезано кодом: 70 → 58 мин
  (…)» — видно, что предлагал LLM.
- **Объём недели держит код; в safety-разгрузку объём плоский (решение владельца 06.09.2026).**
  План 07–13.09 после пересборки суммировал ≈ 30 км при цели 27,9 (+18 % к прошлой неделе) — `target_km`
  уходил LLM только числом. `planning_safety.cap_week_volume` ужимает лёгкие/восстановительные дни
  пропорционально до цели (допуск `WEEK_VOLUME_TOLERANCE_PCT` 5 %, пол `PLAN_EASY_MIN_MINUTES` 25 мин),
  длительную и качество не трогает; заметка «⚠️ Объём недели урезан до ~N км…», `meta.week_volume_capped`.
  `apply_safety_to_targets` при закрытом интенсиве ставит `target_km = prev_week_km`, пересчитывает
  `long_run_km_max`, флаг `volume_held_by_safety` → шапка «…без интенсива (safety), объём без роста».
  Цикл финализации плана — два прохода (predicted → потолки → повторный `finalize` → запись).
- **Сегменты элементов плана недели терялись.** `weekly_plan` собирал
  `WorkoutProposal` без `segments` (схема их допускала, чат — использовал), поэтому обещанные в прозе
  ускорения до карточки не доходили (LLM ставил зону 4 всему дню, safety срезал). Теперь
  `segments_from_schema`, как в чате; `PLAN_PROMPT` задаёт формат ускорений сегментами и запрещает
  зону 3–4 всему дню.
- Заметки планировщика (длительная, объём, беговые дни) — внутри карточки до футера
  (`render_week_plan(notes=…)`), а не после «Перепланировать: /plan».
- **Потолок длительной держит код, не промпт.** План 07–13.09 дал длительную 70 мин ≈ 10 км при
  потолке 30 % недели (8,4 км) и «без роста длительной» в отчёте — `long_run_km_max` уходил LLM только
  как число в `week_targets`. Теперь `planning_safety.cap_long_run` при финализации плана урезает
  длительную по тому же ориентиру км, что печатает карточка (`predicted.distance_km`, допуск
  `LONG_RUN_CAP_TOLERANCE_KM` 0,3 км), и по `long_run_min_max` (150 мин); повторный `finalize`
  (Prescription — только из `clamp`); строка «⚠️ Длительная урезана до N мин (≈X км): потолок 30 %
  недельного объёма» под карточкой, `meta.long_run_capped`. Без истории темпа — только потолок минут.
- **Отложенные обещания не замалчиваются.** Просьба подопечного от 04.09 («темповые вставки на
  следующей неделе») выпала из 8-ходового окна истории, и план её проигнорировал молча.
  `turn_context.recent_athlete_requests` (реплики подопечного за 7 дней) → `extras["athlete_requests"]`
  плана; `PLAN_PROMPT` велит одной фразой объяснить, что и почему откладывается.
- **Ускорения при закрытом качестве не запрещены.** Фраза «только лёгкие и длительная» при
  `hard_days_max = 0` заменена: короткие ускорения 15–20 с (4–6, свободно) в конце одного лёгкого дня
  как сегменты допустимы — safety их качеством не считает (гайды 45/46, `is_stride`).
- **Урезанная качественная работа больше не становится «длительной» той же длины.** Инцидент
  06.09: в плане недели LLM заложил темповую (Z3, 40 мин), safety закрыл интенсив (правила 16/17 —
  31% времени в Z3+ за 7 дней, лёгкие слишком быстро), а `safety._downgrade` выбрал «самый интенсивный
  разрешённый тип» → `tempo → long` → карточка показала «🟦 Длительный бег 40 мин», неотличимый от
  лёгкого. Теперь `long` — кандидат даунгрейда только для самого `long`; tempo/interval/race → `easy`.
  Карточка недели называет замену и причину по дням («⚠️ Чт 10.09: 🟠 Темповая → 🟢 Лёгкий бег — за 7
  дней 31% времени в Z3+», `render_week._clamp_notes`; общая строка «Часть дней урезана» — только для
  урезанных зоны/длительности); `safety.rehydrate` восстанавливает `proposal` из `proposal_json`, чтобы
  и `/week` показывал замену. Потолки недели видят вердикт **до** промпта: новый
  `coach/planning_safety.apply_safety_to_targets` обнуляет `hard_days_max`/`quality_*_km_max` при запрете
  интенсива и пишет `quality_blocked_by_safety`; шапка карточки — «… (рост) · цель ~N км · без интенсива
  (safety)»; `PLAN_PROMPT` — при `hard_days_max = 0` не обещать темповую в прозе. Хвосты — BACKLOG
  #315–317.

### Added
- **Бот отписывается, когда синк снова работает.** После алерта «⚠️ Синхронизация … не работает»
  (3-й сбой подряд) первый успешный синк шлёт «✅ Синхронизация … восстановлена»: сколько было сбоев,
  длительность паузы от последнего успеха, сколько подтянуто (или «новых … не было»). Признак «алерт был» —
  счётчик `*_sync_failures ≥ SYNC_FAILURE_NOTIFY_THRESHOLD` на момент успеха, новых колонок нет.
  Тексты вынесены в `_sync_failed_text`/`_sync_recovered_text` (`services/sync/orchestrator.py`);
  ручной синк из web (`run_sync_for_user`) тоже закрывает алерт. Повод — сбой Coros 06.09 (504 на логин,
  «系统维护中»). Тесты — `tests/test_auto_sync.py`. Хвосты — BACKLOG #312, #313.
...
в проде за 60 дней три часовые «long»-по-плану помечены easy, четыре лёгкие — tempo при пульсе на
границе Z2/Z3. Ярлык расходился по всему коучу. Решения владельца 04.09.2026: план решает назначение
(recovery/easy/long, tempo/interval/race), факт — интенсивность; история переразмечается; сырой
ярлык хранится отдельно.

### Added
- **`src/analysis/type_resolution.py::resolve_training_type`** (чистая): интервальная структура —
  всегда interval; пульс ≥ порога качества (`is_quality_session`) — tempo/тип плана (race — первый
  путь к этому ярлыку); без качества — назначение по плану (long при ≥ `LONG_RUN_MIN_MINUTES`=60 или
  ≥ `LONG_RUN_PLAN_RATIO`=0.8 плановой, recovery при пульсе ≤ восстановительного, иначе easy); без
  плана — long по длительности, иначе easy (catch-all tempo устранён).
- **Колонки `training_sessions.training_type_auto` / `training_type_source`** (auto|plan|manual) —
  миграция `v5w6x7y8z9a0` (аддитивно, backfill auto=текущий ярлык). `process_trackpoints` отдаёт
  провенанс; `reanalyze` сохраняет сырой ярлык и ставит manual при override.
- `workout_insights.apply_type_resolution` — вызывается в `upsert_workout_insights` до метрик
  (plan_vs_actual/long_run/peers видят честный тип); идемпотентно, override не трогает.
- `services/type_resolution_backfill.relabel_sessions` — переразметка истории с пересчётом разборов.
- `_session_brief`: `type_source`, `is_quality` (#311 — интенсивность по факту, не по ярлыку).
- Тесты: матрица резолвера (21 кейс), relabel в upsert (план/без плана/override), провенанс в
  `process_trackpoints`, `week_done` по effective-типу, backfill.

### Changed
- `planning_window.week_done`, `planning.week_plan_review` — `effective_training_type` (override
  учитывается). Web: тип с источником «по плану тренера / вручную / авто», `race` в словаре.
- `LONG_RUN_MIN_MINUTES` — единый источник `src/config/constants.py` (coach/config импортирует).
- BACKLOG: #290 закрыт → архив; #222 — сырой ярлык накапливается для разметки.

## [04.09.2026] — Safety: интенсивность по сегментам, длительная = качественный день, правило перекоса недели

Инцидент 04.09 06:51 (recommendations #38): модель предложила «лёгкий бег» с отрезками 4×3 мин в Z3
(до ПАНО) через день после длительной, на 5-й беговой день, при 69% лёгкого времени за неделю —
пороговые интервалы под ярлыком easy. Гейт «интенсив не раньше …» смотрел только на ярлык
(`HARD_TYPES`) и не сработал; карточка противоречила себе («Интенсив — не раньше 09:45»).

### Added
- `safety.effective_workout_type(proposal)` / `is_stride(seg)`: тип по содержимому сегментов —
  рабочие отрезки дольше `STRIDE_MAX_SEC=30` с в Z3 → tempo, Z4+ → interval. `clamp()` шаг 2b
  переклассифицирует предложение до гейтов (rationale «тип easy → tempo: отрезки 4×3 мин в Z3 —
  качественная работа, не ускорения»); при срабатывании гейтов — даунгрейд до easy, `finalize`
  отбрасывает сегменты (`proposal_type=effective`), карточка — ровная + блок ограничения; без
  гейтов назначение честно становится tempo.
- Правило 16 p1_safety `week_intensity_overload`: `signals.hard_share_7d` (доля Z3+ за 7 дней из
  `zone_distribution(days=7)`, при ≥ `HARD_SHARE_MIN_MINUTES_7D=60` мин) > `HARD_SHARE_OVERLOAD`
  → `max_zone=2`, без интенсива (гайд 10; замыкает часть #289; решение владельца 04.09.2026).
- Тесты: таблица `effective_workout_type`, регресс #38 на уровне clamp и `finalize` (сегменты
  отброшены, блок ограничения), ускорения остаются easy, правило 16, длительная в `_week_signals`,
  `hard_share_7d`.

### Changed
- `state._week_signals`: длительная — качественный день для правила 12 «минимум один лёгкий день
  между качественными» (Дэниелс, гайд 45: К3; решение владельца 04.09.2026). Длительная — по ярлыку
  `long` ИЛИ по длительности ≥ `LONG_RUN_MIN_MINUTES=60` (классификатор часовую пробежку 03.09 пометил
  easy, #290).
- Промпт `segments`: ускорения — 15–30 с; отрезки дольше в Z3+ — workout_type tempo/interval.
- `render_prescription`: «… с ускорениями» только когда все рабочие отрезки короткие.
- METRICS_GUIDE §7: строки правил 16 и «интенсивность по сегментам»; CLAUDE.md блок коуча.

## [04.09.2026] — Коуч: гвард отменённых дней (инцидент «пробежка на отменённую пятницу»)

Инцидент 04.09 06:51: после `/report` подопечный ответил на вопрос о приоритетах следующей
недели — чат истолковал ответ как «что делать сегодня» и назначил пробежку на пятницу, которую
подопечный накануне отменял («пт-сб-вс не смогу»); строк отдыха на пт/сб не было (отмена была до
появления механизма, вечером модель сняла только вс), сообщение выпало из окна истории.

### Added
- **Маркер `UNAVAILABLE_RATIONALE`** (`coach/config.py`): `planning.cancel_days` пишет его в
  `proposal_json.rationale` rest-строки; `turn_context.is_athlete_unavailable(row)`;
  в `planned_workouts` поле `athlete_unavailable: true`.
- **Детерминированный гвард** `planning.blocked_by_unavailable`: в чате и утре LLM-предложение
  тренировки на день, отменённый подопечным, отбрасывается; в тексте — «На Пт 04.09 ты говорил,
  что бегать не сможешь — назначение не ставлю…»; утро подтверждает отдых.
- **Обратный путь** `CoachTurn.available_again_days_ahead` → `planning.reopen_days` («в субботу
  всё-таки смогу» → отдых-отмена гасится, текст «Снял отдых: …»).
- Тесты: гвард в чате (и что обычный rest-план не блокирует), reopen, утро с отменённым отдыхом,
  `cancel_days` пишет маркер.

### Changed
- Промпт `proposal`: ответ на вопрос отчёта / разговор о целях и приоритетах предстоящей недели —
  не назначение (`proposal=null`); день с `athlete_unavailable=true` не назначать.
  Промпт `unavailable_days_ahead`: «последняя тренировка на неделе / больше на этой неделе не
  бегаю» → все оставшиеся дни недели, не один названный.
- Данные прода 04.09 07:12: строка #38 (easy пт) погашена, отдых на пт/сб (#39/#40) через
  `cancel_days`; маркер проставлен на #37/#39/#40 после деплоя.
- BACKLOG #311 — проза отчёта «интенсивные дни» против `quality_runs=0`.

## [03.09.2026] — Коуч C8.1: недельный отчёт v2 — карточка чисел + интерпретация, `/report`

Цель владельца: отчёт должен мотивировать, показывать прогресс, называть слабое место и направление
подготовки — не «писать ради письма». Прежний отчёт был чистой прозой LLM без карточки чисел
(модель сама читала `weeks[]`, доля лёгкого — по типу тренировки, недели по UTC, целей недели в
контексте не было). Решения владельца 03.09.2026: полная карточка (9 строк), только про тренировки
(без HRV/RHR/сна — они в утреннем вердикте), сравнение — прошлая неделя + среднее за 4 + ряд 6
недель, команда `/report`.

### Added
- **`src/coach/week_report.py`** — `compute_week_report`/`build_week_report`: числа недели по
  ЛОКАЛЬНОЙ дате тренировки (полные недели пн–вс, не UTC-корзины `weekly_volume`): км, минуты,
  пробежки, качественные (`is_quality_session`), длительная и её доля, **доля лёгкого ВРЕМЕНИ по
  зонам** (посекундные `time_in_zones` из разборов, fallback — сегменты), **недельная сумма баллов
  нагрузки** Дэниелса (обещана в METRICS_GUIDE M1.4), **экономичность** — среднее `hr_vs_baseline.
  delta_bpm` за неделю (главный маркер прогресса по Лидьярду), дни с болью, флаги разборов, ACWR,
  цели недели (`week_targets_stored`), `next_week` (week_targets следующей — только в вс), сверка
  с планом. **`highlights`/`concerns`** — сигналы прогресса/тревоги предвыбирает код по порогам
  `coach/config` (LLM выбирает из них, не выдумывает).
- **`src/coach/render_week_report.py`** — карточка «Итоги недели»: объём/цель, к прошлой и среднему,
  лёгкое время, качество/длительная, нагрузка/ACWR, экономичность, план недели, ряд 6 недель;
  строки без данных опускаются; ✓/⚠ по порогам.
- Константы `WEEK_REPORT_SERIES_WEEKS=6`, `WEEK_REPORT_AVG_WEEKS=4`, `EFFICIENCY_GAIN_BPM=-2`,
  `EFFICIENCY_LOSS_BPM=3`, `HARD_SHARE_OVERLOAD=0.30`, `WEEK_REPORT_ACWR_HIGH=1.3`,
  `EASY_TOO_HARD_WEEK_FLAGS=2` (`coach/config.py`; зеркало — METRICS_GUIDE §12).
- Команда **`/report`** — итоги текущей недели (пн–сегодня) по запросу.
- `numeric_check.prose_numbers` — для kind=weekly числа в прозе → лог + `meta.numeric_mismatch`.
- Тесты: `test_week_report.py` (корзины через полночь/UTC, зоны+fallback, баллы, экономичность,
  сигналы, пустая неделя), `test_render_week_report.py`, обновлён `test_review_weekly.py`
  (карточка в LLM-пути и fallback, `week_report` в контексте, числа в прозе → meta, джоб отдаёт
  числа и отчёту, и плану), `test_planning` (`include_today`).

### Changed
- `orchestrator.weekly_report(report=)`: текст = проза LLM + карточка + вопрос; `weekly_summary`
  убран из контекста (дубль); fallback без LLM — карточка + одна строка. `_llm_chat_turn(extra_card=)`.
- `WEEKLY_PROMPT` переписан: интерпретация, не пересказ — один сигнал прогресса из `highlights`,
  одно слабое место из `concerns`, направление по `next_week`, ≤5 предложений, без чисел и
  дежурной похвалы. `PLAN_PROMPT`: одной фразой — что меняется относительно прошедшей недели.
- `jobs/coach_weekly.py`: числа недели считаются один раз и передаются и отчёту, и плану
  (`generate_weekly_plan(week_report=)`).
- `planning.week_plan_review(week_start=, include_today=)` — в отчёте вс 19:00 воскресенье без
  тренировки считается пропущенным.
- BACKLOG: #308 монотонность/страйн Фостера, #309 тренды каденса/экономичности в web, #310
  кросс-тренинг в 80/20.

## [03.09.2026] — Коуч: строка «Изменил план на … (было: …)» и короткие дни Пн…Вс

Хвост отмены дня «Снял назначения (отдых): во 06.09» читался как «снял отдых», дни недели резались
до двух букв («по», «че», «пя», «су»). Решения владельца 03.09.2026: формат «Изменил план на Вс 06.09:
🛌 Отдых (было: 🟦 Длительный бег · 80 мин)» без зачёркивания (legacy-Markdown Telegram его не умеет);
показывать при любой замене дня; сокращения Пн Вт Ср Чт Пт Сб Вс — в строках списков, заголовки
карточек дня остаются полными («… — воскресенье 06.09»).

### Added
- `render_week.plan_change_line(when, new, old)` — детерминированная строка замены назначения;
  без прежней строки — «Поставил на Вс 06.09: …». Вызывается в трёх местах: отмена дней
  (`planning.cancel_days`, по строке на дату), утренний вердикт со сменой плана (`adjusted`,
  `confirm_or_adjust_morning` теперь возвращает и `plan_row`), чат, заменивший уже данное назначение
  (`orchestrator._llm_chat_turn`). Строка идёт над новой карточкой.
- `planning.latest_rows_for_dates` — последняя действующая строка на дату (для «было: …»).
- `timeutils.WEEKDAYS_RU_SHORT` — единый источник коротких дней; карточка `/week` использует его.

### Tests
- `test_plan_change_line_formats`, `test_chat_replacing_today_row_shows_change_line`,
  `test_latest_rows_for_dates_skips_superseded`; ожидания дней в карточке недели → «Пн/Вт/Ср/Вс».

## [03.09.2026] — Коуч: отмена дней плана из чата (инцидент «воскресная длительная»)

Инцидент 03.09: «нужно переделать план на неделю: пт-сб-вс не смогу» ушло обычным чат-ходом;
LLM записала только назначение на сегодня, «воскресную отменяем» осталось прозой — строка
воскресенья в `recommendations` жила дальше, и разбор/вечерний чат снова ждали «воскресную».

### Added
- **`CoachTurn.unavailable_days_ahead`** (`llm/schemas.py`, 0..7) + правило в `OUTPUT_CONTRACT`:
  дни, когда подопечный не сможет бегать. Оркестратор (chat/morning) → `planning.cancel_days`:
  строки без факта на эти даты → `superseded`, записывается `rest` со status `adjusted`,
  в текст — детерминированный хвост «Снял назначения (отдых): …». `planned_workouts` и `/week`
  показывают отдых. `planning.supersede_rows_for_dates` — гашение по конкретным датам.
- Тест `test_unavailable_days_cancel_planned_rows` (строки с фактом не трогаются).

### Changed
- `_REPLAN_RE` (`telegram/handlers/coach.py`) ловит «переделать/изменить/поменять/скорректировать
  … план» → `/plan` (остаток недели с гашением прежних строк); «изменилось самочувствие, план держу»
  не матчится. Тесты в `test_replan_regex.py`.
- `SYSTEM_PERSONA`: явный род — подопечный мужчина, тренер говорит о себе в мужском роде
  (в переписке 02–03.09 плавало: «поняла/учла» vs «рад слышать», «ты держалась»).
- BACKLOG #305 (утро подтверждает устаревшую строку плана), #306 («Интенсив — не раньше» шумит/дрейфует).
