...
## [19.09.2026] — Кэпы дня меряли воскресенье числами СЛЕДУЮЩЕЙ недели: длительная резалась вдвое

Вопрос владельца («отменил пятницу — пересчитает ли бот воскресную?») привёл к симуляции утра 20.09 на
проде: вердикт урезал бы плановую длительную **75 → 46 мин**:

```
targets: week 2026-09-21  target_km 16.6  long_run_km_max 6.6
⚠️ Длительная урезана до 46 мин (≈6.6 км): потолок 40 % недельного объёма
```

Причина: в воскресенье `plan_window` уходит на следующую неделю (это нужно `/plan` в 19:00), а ad-hoc путь
чата/утра берёт те же числа через `day_caps.day_targets`; `cap_long_run` — единственный кэп без проверки
«день внутри недели targets». Отменённая пятница усугубила: следующая неделя считается от недобранной
текущей (15.1 км → target 16.6 → потолок длительной 6.6 км). Кэпы в ad-hoc пути появились 16.09 (#338–#343),
прошлое воскресенье было 13.09 — поэтому баг всплыл бы впервые 20.09.

### Fixed
- **Ad-hoc путь считает неделю, которой принадлежит день**: `plan_window(..., current_week=False)` —
  при `current_week=True` воскресный скачок на следующую неделю не делается; `week_targets` прокидывает
  флаг; `day_caps.day_targets` зовёт с ним. `/plan` и воскресный джоб — без флага, их семантика прежняя.
- **Гвард в `cap_long_run`**: дата назначения вне недели `targets` → кэп не применяется (тот же принцип,
  что у `cap_day_volume`). Общий helper `planning_window.week_bounds` (был приватным в `day_caps`).

### Tests
- `tests/coach/test_day_caps_week_scope.py` (8): окно с флагом и без; `week_bounds`; гвард `cap_long_run`
  внутри/вне своей недели; регрессия — `day_targets` в вс описывает текущую неделю и длительная 75 мин
  не режется; будний день по-прежнему режется. Всего 1181 зелёный.

### Решение владельца
- Добор объёма после отмены дня **не вводим**: пропущенный день остаётся пропущенным, остальные дни не
  растут (гайды против наращивания плана из-за пропусков). `/plan` среди недели по-прежнему может выдать
  более длинный остаток — это существующее поведение.

### Backlog
- #363: ad-hoc предложение в вс вечером на СЛЕДУЮЩУЮ неделю кэпами объёма не проверяется (targets —
  текущая неделя, гвард по границам их пропускает); полное решение — считать кэпы по неделе даты
  предложения, а не одного `now`.

## [19.09.2026] — План на день приходит после скриншота сна, 09:30 — резерв

Решение владельца: сон решает, делать ли сегодняшнюю тренировку (особенно качественную), поэтому вердикт
должен считаться уже со сном. Скриншот приходил раньше 09:30, но напоминание о нём стояло в 10:00 — ПОСЛЕ
вердикта, и данные сна в план дня попадали случайно.

### Changed
- **Напоминание о скриншоте сна — 09:00** (было 10:00): за 30 мин до резервной отправки (`telegram/main.py`,
  `jobs/sleep_reminder.py`).
- **Вердикт уходит сразу после распознанного скриншота сна за сегодня** (`handlers/sleep_photo` →
  `jobs/coach_morning.maybe_deliver_after_sleep`), без нижней границы по времени; джоба 09:30 —
  РЕЗЕРВ для дней без скриншота. Дедуп — заявка дня в `UserModel.params_json["morning_verdict"]`
  (`coach/morning_state.py`, без миграции): резерв не дублирует ранний вердикт.
- **Поздний скриншот** (вердикт уже ушёл БЕЗ сна) → один пересчёт с префиксом «🔄 С учётом сна:»
  по отдельному `MORNING_SLEEP_PROMPT`, не позже `COACH_SLEEP_RECOMPUTE_STOP_HOUR` (12). Пересчёт слово
  в слово повторил утренний вердикт → вместо копии строка «Сон учёл — план дня не меняется».
- **Ход 09:30 ещё в полёте** (скрин в 09:31) → одна отложенная перепроверка `_sleep_recheck_job`
  (`MORNING_SLEEP_RECHECK_DELAY_S` 90 с), а не тишина.

### Added
- `src/coach/morning_state.py` — заявка/пометка отправки за день: `plan_action` (чистая: send/resend/
  wait/skip), `claim`/`mark_sent`/`release`; заявка берётся синхронно из корутины (event loop не
  переключается) и протухает через `MORNING_CLAIM_STALE_MIN` (15 мин) — креш контейнера не съедает день.
- Пометка **«данных сна нет»** в вердикте — только перед качественным днём (решение владельца; в лёгкий
  день/отдых сон влияет слабо): `render.render_sleep_missing_note` + `planning_rows.hard_day_on`,
  приклеивается через новый параметр `handle_chat(suffix=…)` — и в LLM-ходе, и в детерминированном
  fallback, до персиста (история совпадает с отправленным).
- `services/sleep_ingest.has_sleep_for_date` — общий источник правды «скрин сна за дату есть»
  (напоминание 09:00 переведено на него).

### Fixed
- **Повторный утренний ход больше не переписывает `adjusted` в `confirmed`** (`planning_rows.py`):
  статус подтверждается только у строк `planned`/`proposed`, иначе пересчёт со сном стирал факт
  осознанной замены дня (недельный отчёт и `/plan` видели неверную картину).
- **Исчерпанный дневной бюджет ходов не съедает вердикт**: `handle_chat(kind="morning")` при
  `turns_today >= COACH_MAX_TURNS_PER_DAY` отдаёт детерминированный `morning_verdict()`, а не отписку
  «лимит разговоров исчерпан» (её джоба отправляла как план дня).

### Tests
- `tests/coach/test_morning_state.py` (11) — решения по скриншоту, перехват протухшей заявки, дедуп
  повторов, сохранность соседних ключей `params_json`.
- `tests/coach/test_morning_sleep_trigger.py` (9) — скрин → вердикт и молчание резерва; скрин за вчера;
  поздний пересчёт с префиксом и его дедуп; стоп-час; ход в полёте; инициатива off.
- `tests/coach/test_morning_sleep_note.py` (8) — пометка только перед качественным днём (LLM и fallback),
  `hard_day_on` против `hard_day_planned`, бюджет ходов.
- Обновлены `test_morning_job.py` (фейк хода принимает `prompt`/`suffix`) и `test_morning_confirm.py`
  (отдых из-за недоступности остаётся `adjusted`). Всего 1173 зелёных.

### Backlog
- #359: поздний пересчёт может ужесточить день не из-за сна — `latest_metrics` берёт строку с максимальной
  датой, и «пустая» сегодняшняя `DailyMetrics` из скриншота прячет вчерашние HRV/recovery.
- #360: нет catch-up вердикта после рестарта бота между 09:30 и скриншотом.
- #361: джобы живут в `settings.timezone`, локальные даты — в `user.timezone` (мульти-юзер).
- #362: `test_day_caps.py::test_chat_day_volume_respects_other_planned_days` датозависим (падает в субботу
  на нетронутом `main`).
