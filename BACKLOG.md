...
| 319 | Среди недели вечером (после ~20:00 локально) `plan_window` считает день 0 доступным, если сегодня не бегали → `/plan` ставит тренировку на уходящий день (07.09: карточка в 18:00 с «▶ Пн — Лёгкий бег»). Порог часа — в `coach/config.py`. | S |
| 320 | Тесты зависят от дня недели запуска: фикстура `athlete_with_history` кладёт сессии `i*2` дней назад, границы недели плавают → в понедельник 07.09 падают `test_weekly_plan.py` (3: persists_rows, midweek_rest_of_week, caps_week_volume) и `test_planning.py` (week_plan_review_done_missed, detraining_return_ceiling). Якорить фикстуру к пн/вс. | S |
