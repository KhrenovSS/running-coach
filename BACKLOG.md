...
| 344 | [Tests] | **`tests/coach/test_planning.py::test_week_targets_long_run_hold_after_share_flag` падает во вторник** (найдено 16.09.2026 на нетронутом `main`): длительная 9 км «3 дня назад» и 15 км «9 дней назад» относительно `utcnow()` попадают в разные недели в зависимости от дня недели — `_last_long_run_km(since=week_start − 7)` видит не ту тренировку (`long_run_km_max` 9.6 > 9.0). Сделать датонезависимым (якорь понедельника, как #328/#332). | `tests/coach/test_planning.py:340-356` | ⬜ найдено 16.09.2026 |
