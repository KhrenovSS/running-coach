...
(последняя — 10.09.2026: 248 закрытых строк; нумерация сквозная, следующий свободный номер — #331).
...
**Рекомендуемый порядок первого спринта:** ~~249 → 289 → 308 → 220 → 292+305 → 294 → 295 → 311~~ → 264 → 263 → 286~~ (закрыты 04.09.2026) → ~~259 + 289~~ (закрыты 08.09.2026); #243 ждёт даты старта; дальше P2: ~~#253 → #302~~ (08.09), #247 v2 → #237; по пути — #328 (датозависимый тест), #329 (файлы > 400 строк).
...
| 329 | [Arch] | **Четыре файла превышают лимит ~400 строк** (CLAUDE.md §1): `src/coach/planning.py` 532, `src/services/workout_insights.py` 488, `src/coach/orchestrator.py` 435, `src/analysis/utils.py` 404 (аудит документации 10.09.2026). Разнести логику по модулям (planning → planning_*; orchestrator → отдельный модуль сценариев; utils → elevation/pace). | `src/coach/planning.py`, `src/services/workout_insights.py`, `src/coach/orchestrator.py`, `src/analysis/utils.py` | ⬜ |
| 330 | [CI] | **CI гоняет Python 3.12, Dockerfile — 3.13** (`.github/workflows/ci.yml` `python-version` vs `Dockerfile:1 python:3.13-slim`): тесты не покрывают прод-интерпретатор. Поднять матрицу до 3.13 (аудит документации 10.09.2026). | `.github/workflows/ci.yml` | ⬜ |
| 331 | [UX] | **Прошедший день отдыха в карточке недели рендерится как «✗ … 🛌 Отдых · пропущен»** (`render_week._fact_line`: без связанной тренировки любой прошедший день — «пропущен»). Для rest нужен нейтральный маркер («отдых» без ✓/✗), для отмены подопечным — «по твоей просьбе», как у будущих дней (11.09.2026). | `src/coach/render_week.py` | ⬜ |
| 332 | [Tests] | **`test_weekly_plan.py::test_weekly_plan_keeps_strides_segments` падает на чистом дереве** — `sqlalchemy.exc.MultipleResultsFound` (замечено 11.09.2026 при правке карточки недели; воспроизводится и без изменений). Найти `.one()`/`scalar_one()` по пути `generate_weekly_plan` → сегменты, починить выборку или фикстуру. | `tests/coach/test_weekly_plan.py`, `src/coach/weekly_plan.py` | ⬜ |
