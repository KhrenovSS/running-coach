...
| 314 | [Tests] | **Два теста падают на чистом `main` (06.09.2026)**: `test_docs_links` — BACKLOG ссылается на `~/.claude/plans/iterative-weaving-scone.md` (файл вне репо); `test_week_view::test_latest_row_per_date_wins_and_today_marked` — датозависим (в воскресенье ожидает «80 мин», карточка показывает только сегодня). Починить ссылку, зафиксировать дату в тесте. | `BACKLOG.md`, `tests/coach/test_week_view.py` | ⬜ |
