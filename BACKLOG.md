...
(последняя — 02.09.2026: 209 закрытых строк; нумерация сквозная, следующий свободный номер — #329).
...
**Рекомендуемый порядок первого спринта:** ~~249 → 289 → 308 → 220 → 292+305 → 294 → 295 → 311~~ → 264 → 263 → 286~~ (закрыты 04.09.2026) → ~~259 + 289~~ (закрыты 08.09.2026); #243 ждёт даты старта; дальше P2: ~~#253 → #302~~ (08.09), #247 v2 → #237.
...
| 233 | [Tests] | `tests/test_health.py:7` — `os.environ.setdefault("SECRET_KEY", ...)`: CI grep-гвард по паттерну `os.environ.setdefault` может его зацепить (это не DATABASE_URL, но паттерн совпадает). Заменить на явное присваивание. Найдено db-safety-ревью 05.08.2026. 08.09.2026: тот же паттерн в `tests/test_logs_route.py:3`. | `tests/test_health.py:7`, `tests/test_logs_route.py:3` | ⬜ |
...
| # | Тег | Описание | Файл / Источник | Статус |
|---|-----|----------|-----------------|--------|
| 302 | [Analysis] | **Сегментация коротких интервалов — проверка на реальной тренировке** (из архивного PROJECT_AUDIT, AUDIT-014 DoD): применить `segment_by_pace()` к реальной интервальной тренировке (бывшая session id=67 — перезагрузить TCX или взять №33 от 05.08) и сверить число сегментов с лапами часов. | `src/analysis/segment.py`, `tests/` | ✅ 08.09.2026 (решение владельца): переформулирован в фичу — сегментация по структурным лапам часов (`analysis/segment_laps.py`, гейт `structural_laps`; тестовый случай — тренировка 01.09: 7×18 с ускорений, лапы программы часов). Проверка темповой эвристики на короткой интервалке не требуется: на структурных тренировках лапы авторитетны, эвристика остаётся для авто-км/TCX (#109/#113) |
...
| 324 | [Tests] | **`test_illness.py::test_week_targets_exclude_paused_days` зависит от дня недели**: ожидает ≥ 6 `unavailable_dates` при паузе 7 дн, но `week_targets` кладёт в `availability` только даты остатка текущей недели — в понедельник их 5 (падает 08.09.2026 на нетронутом дереве; в четверг проходит). Либо считать даты паузы за границу недели, либо ослабить ожидание до `min(pause, дней до вс)`. | `tests/coach/test_illness.py:78`, `src/coach/planning.py` | ⬜ |
...
| 326 | [Data] | **Open-Meteo: день запроса — локальная дата, часы — UTC** (`analysis/__init__.py:226`, `timezone=UTC`): пробежка в 01:00 МСК (22:00 UTC накануне) ищет ближайший час в UTC-сутках локальной даты → берётся 00:00 UTC (03:00 МСК) вместо 22:00. Запрашивать `start_date = end_date − 1` … или `timezone=auto`. Низкий приоритет (ночных пробежек нет). | `src/analysis/__init__.py`, `src/parsers/weather.py` | ⬜ |
...
| 328 | [Tests] | **`test_illness.py::test_week_targets_exclude_paused_days` зависит от дня недели**: `unavailable_dates` считается по остатку текущей недели (#293), в четверг это 3 дня < `min(pause, 6)` → падает и на чистом дереве (10.09.2026). Якорить `today` к понедельнику или считать ожидание от `days_ahead_allowed`. | `tests/coach/test_illness.py:78` | ⬜ |
