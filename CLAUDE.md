...
- **База знаний (E2/E2.1)**: гайды `knowledge/guides/` — seed 00–30, Фицджеральд 40–42/60, Дэниелс
  ...
  `test_guide_queries.py`), проза — чанки по запросам `loader.review_guides_queries` (боль → тип →
...
- **Порядок работ — `BACKLOG.md`, раздел «Приоритеты»** (P0 безопасность → P1 нагрузка/планы → P2
  данные → P3 приложение). Состояние на 08.09.2026: P0 и P1 закрыты (#259/#289 — 08.09), кроме
  #243 (ждёт даты старта); следующий кандидат — P2: #247 v2 (обрезание прозы при
  расхождении с числами), затем #237; 08.09 закрыты #253 (набор высоты — гистерезис 2 м) и #302 (сегменты по
  структурным лапам часов — `analysis/segment_laps.py`; интервалы и ускорения бегать программой на часах).
