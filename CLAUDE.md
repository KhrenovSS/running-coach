...
- **⏩ ТЕКУЩАЯ ТОЧКА ВОЗОБНОВЛЕНИЯ — `docs/coach/TASK_2026-09-13_athlete_requests.md`, §5**:
  13.09.2026 найдено (кода НЕ трогали, решение владельца — правки ведёт Fable после обнуления лимитов, и это
  первое, с чего начинается следующий заход): **детерминированной оценки просьбы подопечного изменить тренировку
  нет вообще**. В чате/утре `clamp` режет объём только при боли или сне < 5 ч (`max_duration_min` = 40), иначе поле
  `null` — просьба «хочу 30 км» проходит насквозь; потолки недели (`cap_long_run`, `cap_week_volume`, `week_targets`)
  вызываются только из `/plan`, и LLM их в чат-контексте даже не видит. BACKLOG #338–#343, P0 снова открыт.
  Требование владельца: уменьшить/перенести/отменить — уже работает, «предложить больше» — должно оцениваться,
  а не приниматься слепо. **Fable проводит свой независимый разбор и предлагает решение владельцу ДО правок**
  (§0 документа): §5 — черновик направлений, не утверждённый дизайн.
...
- **Порядок работ — `BACKLOG.md`, раздел «Приоритеты»** (P0 безопасность → P1 нагрузка/планы → P2
  данные → P3 приложение). **13.09.2026: P0 снова открыт — #338–#343** (нет потолка объёма в ad-hoc пути).
...
| ⏩ Просьбы подопечного об изменении тренировки — нет оценки объёма (#338–#343, 13.09.2026) | `docs/coach/TASK_2026-09-13_athlete_requests.md` |
