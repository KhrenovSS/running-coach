...
- **Тренировки по сегментам (M2.1, 01.09.2026)**: `WorkoutProposal.segments` +
  ...
  `render_segments.py` (компактная карточка, общий итог времени считается ИЗ сегментов). Нормативный
  темп по зонам (VDOT/ПАНО) и экспорт тренировки в Coros — BACKLOG #273/#272.
...
- **Чек-листы C0–C9 и D0–D8 закрыты 25.08.2026. Дальше (§9 DEV_PLAN): гигиена ✅ 29.08
  ...
  тренировки по сегментам (назначение, «M2.1» в коде) ✅ (01.09), устойчивость моста ✅ (01.09),
  квалиметрия GPS + оценка дистанции по шагам при сбое ✅ (01.09, `analysis/gps_quality.py`,
  METRICS_GUIDE §6.1).
  Осталось: персонализация (#244/#246, ждёт накопления insights), правило «недосып→осторожнее»
  (#254, данные сна теперь есть), план к цели/гонке (#243), **F-серия — сырые данные и
  физиология (утв. 01.09.2026, DEV_PLAN §9)**: F0 фиксы аудита усреднений (#277–#283) →
  F1 парсер FIT v2 (лапы/паузы/эталоны/динамика, #285) → F2 moving-time (#286) ∥ F3 «M2.1
  разбора» (HRR, не путать с назначением по сегментам) → F4 M3.1 (зоны от LTHR/LTSP — уже
  в БД, 🛑 показ зон владельцу) → F5/F6 M4 (#287, METRICS_GUIDE §11).**
