...
- **Недельный план** (`weekly_plan.py` + детерминированные числа `planning.py`; с 11.09 строки плана и утреннее
  ...
  (адаптивно: max за прошлые недели + 1, в [3, 6], `enforce_run_days`). **Метрики разбора — insights (`INSIGHTS_SCHEMA_VERSION` = 12)**: `services/workout_insights.py`
...
- **⏩ ТЕКУЩАЯ ТОЧКА ВОЗОБНОВЛЕНИЯ — потолок объёма и календарь стартов, #243 ч.1 закрыта 17.09.2026 (вечер)**
