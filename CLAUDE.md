...
- **Недельный план** (`weekly_plan.py` + детерминированные числа `planning.py`; с 11.09 строки плана и утреннее
  ...
  (адаптивно: max за прошлые недели + 1, в [3, 6], `enforce_run_days`). **Метрики разбора — insights (`INSIGHTS_SCHEMA_VERSION` = 13)**: `services/workout_insights.py`
  ...
  в `turn_context.py` (записи `recent_reviews` датированы по ТРЕНИРОВКЕ: `date`/`weekday`/`days_ago`, не по
  `created_at` разбора — инцидент 17.09.2026 «вчерашние спуски»; `InsightRepository.recent_with_sessions`).
- **Сон — из скриншота** (Coros API длительность/фазы не отдаёт): пользователь шлёт фото экрана
  ...
  напоминание в 9:00 (`telegram/jobs/sleep_reminder.py`, общий предикат
  `sleep_ingest.has_sleep_for_date`), команда `/sleep`.
- **Утренний вердикт — по скриншоту сна (19.09.2026, решение владельца)**: скрин за сегодня запускает
  вердикт сразу (`handlers/sleep_photo` → `jobs/coach_morning.maybe_deliver_after_sleep`, без нижней
  границы по времени), **09:30 — резерв** для дней без скрина. Дедуп — заявка дня
  `params_json["morning_verdict"]` (`coach/morning_state.py`: `plan_action` send/resend/wait/skip,
  `claim` синхронно в loop, протухает через `MORNING_CLAIM_STALE_MIN`). Поздний скрин → один пересчёт
  «🔄 С учётом сна» по `MORNING_SLEEP_PROMPT` до `COACH_SLEEP_RECOMPUTE_STOP_HOUR`; дословный повтор
  схлопывается в строку. Пометка «данных сна нет» — только перед качественным днём
  (`render_sleep_missing_note` + `planning_rows.hard_day_on`, через `handle_chat(suffix=)`, на обоих
  путях). Утро не отдаёт отписку про лимит ходов: бюджет исчерпан → детерминированный вердикт.
...
- **Safety по содержимому (04.09.2026)**: `safety.effective_workout_type` классифицирует
  ...
  Recovery % — Coros §12 (20/70/90). **Срок «интенсив не раньше» правил 10/11/18/19 — от начала тренировки** (#306,
  ч.2 17.09.2026: сигналы `*_at` в `state.py`, `p1_safety._anchored`; вышел → правило молчит; инцидент 16–17.09: дата
  ехала пт → сб → вс, коуч обещал «интенсив с субботы» на неделе без качественных дней); строка на нехардовой карточке —
  только при качественном дне в плане недели (`planning_rows.hard_day_planned` → `render_prescription(hard_planned=)`),
  промпт: `earliest_next_hard` — граница, не план. Планирование: `long_run_hold`, `detraining_return` (пауза ≥ 14 дн).
...
- **Потолки объёма в чате/утре (#338–#343, 16.09.2026, решения владельца)**: просьба подопечного «пробежать
  больше» оценивается кодом — `coach/day_caps.py` (`day_targets` = `week_targets` + safety; `finalize_with_caps` =
  finalize → `cap_long_run` → `cap_day_volume` → повторный finalize; срез недели в контекст `week_targets (planning)`,
  абзац об объёме в `SAFETY_CONTRACT`), утро сверяет и plan-строку (`confirm_or_adjust_morning(targets=)`).
  **Числа — недели, КОТОРОЙ принадлежит день** (19.09.2026): `day_targets` зовёт
  `week_targets(current_week=True)` — воскресный скачок `plan_window` на следующую неделю только для
  `/plan`; кэпы пропускают дату вне недели `targets` (`planning_window.week_bounds`; инцидент 19.09:
  вс-вердикт резал плановую длительную 75 → 46 мин потолком следующей недели).
  `cap_long_run` — по содержимому (ярлык `easy` не обходит; `race` не режется); структурная выше потолка —
  `coach/segment_trim.py` по гайдам 44/45/46/61 (ускорения: ровная часть вниз, число ускорений ≤ 8 сохраняется,
  < 30 мин ровной → снимаются; качество: повторы вниз до 2, разминка/заминка целы). Сегменты не откатывают
  `max_duration_min` (#343). База сравнения план vs факт — исходная строка дня (`plan_vs_actual.baseline`,
  флаг по максимуму, `INSIGHTS_SCHEMA_VERSION` = 12, #341); порог «длительная < N мин = лёгкая» общий с `/plan` (#342).
  **Допуск по состоянию и диапазон (17.09.2026, решения владельца):** допуск объёма дня в чате/утре —
  `day_volume_tolerance_pct`: 15 % без стойких сигналов усталости/здоровья (`planning_safety.fatigue_signals` =
  `triggered` − `VOLUME_TRANSIENT_SAFETY_RULES`), 5 % при них; гистерезис `DAY_CAP_MIN_CUT_MIN` 10 / `DAY_CAP_MIN_CUT_KM` 1.0
  (короче не режем); `/plan` — 5 %; `cap_long_run` строгий. **Ровный день — диапазон** «30–40 мин · ≈4.3–5.7 км»:
  `coach/volume_range.py`, низ `volume.duration_min_low` = max(30, 75 % верха; long 85 % ≥ порога) считается ТОЛЬКО в
  `prescriber.finalize` по итоговому верху; верх `duration_min` — единственное жёсткое число (кэпы/суммы/экспорт);
  `numeric_check` — интервалы; `plan_vs_actual.within_range`, `volume_ratio` 1.0 внутри; `week_plan_review` сравнивает
  верх/дистанцию; старые строки без низа — точка. `INSIGHTS_SCHEMA_VERSION` = 13.
- **Частота беговых дней в чате/утре (#347/#348, 17.09.2026, решения владельца)**: плановый отдых строкой НЕ
  хранится (`weekly_plan._clean_days`) — «сегодня по плану отдых» = отсутствие строки, гвард `blocked_by_unavailable`
  ловит только отменённые подопечным дни. Лишний беговой день сверх `run_days_max` (факт ∪ действующий план,
  множество дат — `day_caps.run_days_used`, `week_targets.done_dates`) **не отклоняется, а понижается** до `easy` ≤ 2
  зоны ≤ `PLAN_EASY_MIN_MINUTES` без структуры (`cap_run_days`, первый кэп в `finalize_with_caps`; заметка «⚠️ Беговые
  дни недели исчерпаны»); плановые дни (`PLAN_STATUSES`) — каркас, кэп их не трогает, строка `proposed` день не
  освобождает. Контекст: `run_days_max`/`rest_days_min`/`done_runs`/`run_days_used` + `frequency_rule`, абзац о
  частоте в `SAFETY_CONTRACT`. **Правило 22 `run_streak`**: `consecutive_run_days ≥ SAFETY_RUN_STREAK_MAX_DAYS` (3,
  гайды 47/61; серия по локальным датам назад от сегодня в `state._week_signals`) → Z2 без интенсива; сигнал
  сегодняшний — `project_state` обнуляет для дней плана > 0 (частоту будущих дней держит `enforce_run_days`), `PLAN_PROMPT`
  просит не ставить качество (N+1)-м днём подряд; в `VOLUME_TRANSIENT_SAFETY_RULES`.
- **Потолок недельного объёма и календарь стартов (#243 ч.1, 17.09.2026, решения владельца)**: объём растёт +10 %/нед
  (3+1) **до потолка по дистанции цели** — `RACE_VOLUME_CEILINGS_KM` (5 км 40 · 10 км 50 · ПМ 55 · марафон 65, крыша 70,
  без стартов 55 — середина литературы, свод — `docs/coach/TASK_2026-09-17_races_and_volume_ceiling.md`); на потолке —
  `maintenance` (плато, мезоцикл крутится, `capped_by_ceiling` считается ростом для счётчика). **Старты — в чате**:
  LLM сообщает факт `CoachTurn.races` (`RaceReport`: add|cancel, date «ГГГГ-ММ-ДД»|«ММ-ДД», `days_ahead`, км, метка),
  код (`coach/races.py`) подбирает год, валидирует, пишет `params_json["races"]` (top-level; несколько стартов; прошедшие
  гаснут `done` при записи) и отвечает строкой «Записал старт: …». Периодизация — `coach/race_plan.py` (pure):
  `goal_for_week` → `week_targets["goal"]` (потолок = max по стартам в горизонте 18 нед; фазы `race_week` — объём
  max(55 % пика, дистанция), качества нет / `taper` — 75 %, ≤ 1 качественный / `build` — «достижимый пик»
  `peak_capped_by_date` честно в шапке / `base`); `place_race` ставит старт кодом на день гонки (до `enforce_run_days`),
  `races.mark_race` — маркер `target_json["race"]` и на понижённый safety тип («🏁 Старт · в лёгком режиме (safety)»);
  `race` кэпами `day_caps` не режется. Тейпер/неделя старта — не база цикла (`advance_mesocycle`), после недели старта
  мезоцикл с 1; мета несёт `goal` для `/week`. Промпт даты не вычисляет — код. Выносы: `weekly_plan_turn.py`,
  `llm/prompts_plan.py`. **Баг закрыт (#351)**: `advance_mesocycle` стирал `week_plan.availability` каждым `/plan`.
- **Планирование недели (P1 04.09.2026)**: прошлые недели — `planning_window.local_week_volumes`
  ...
  при закрытом интенсиве объём плоский (`target_km = prev_week_km`, 06.09.2026) **только по стойким сигналам**
  (с 12.09 — не для правил 16/17; с 16.09 — и не для `sleep_short`/`sleep_very_short`/`hard_streak`:
  `VOLUME_TRANSIENT_SAFETY_RULES`, `volume_hold`); **мезоцикл 3:1 считает только недели роста** (16.09.2026:
  `week_plan.grew` из `advance_mesocycle`, плоская неделя счётчик не двигает — deload после трёх недель роста);
...
- **Статусы дорожной карты — только в DEV_PLAN §9** (C0–C9, D0–D8, E0–E3, F0–F7 закрыты к 01.09.2026, E2.1 — 07.09;
  ...
  ждёт накопления insights), план к цели/гонке #243 (🟡 17.09.2026: часть 1 — потолок объёма и календарь стартов; остаток — прогноз/тактика/сверка),
...
- **⏩ ТЕКУЩАЯ ТОЧКА ВОЗОБНОВЛЕНИЯ — план на день после скриншота сна, 19.09.2026** (CHANGELOG, верхняя
  запись): вердикт уходит сразу после скрина сна за сегодня, 09:30 — резерв (заявка дня
  `coach/morning_state.py`), напоминание о скрине — 09:00, поздний скрин → один пересчёт «🔄 С учётом сна»,
  пометка «данных сна нет» — только перед качественным днём. Попутно: утро не переписывает `adjusted` в
  `confirmed`, исчерпанный бюджет ходов не съедает вердикт. Код в `main`, тесты зелёные (1173 + известный
  датозависимый `test_day_caps` — BACKLOG #362, падает и на чистом `main`); **закоммичено `fd7e7dc`,
  запушено, задеплоено app+bot 19.09** (бэкап `backup_2026-09-19_20-40-35`; в логе бота — напоминание
  о скриншоте на 9:00, вердикт на 09:30). Прод-проверка: 9:00 напоминание при отсутствии скрина; скрин в 8:xx →
  ack + «🌅 Считаю план на день…» + вердикт, в 9:30 тишина; скрин в 10:00 → «🔄 С учётом сна». Новые
  BACKLOG: #359–#362.
- **Тем же вечером — кэпы дня меряли воскресенье числами следующей недели** (CHANGELOG, верхняя запись):
  `day_targets` → `week_targets(current_week=True)` + гвард по границам недели в `cap_long_run`
  (`planning_window.week_bounds`). Без фикса утро 20.09 урезало бы плановую длительную 75 → 46 мин.
  Решение владельца: добор объёма после отмены дня НЕ вводим. Тесты 1181, новый BACKLOG #363.
  Прод-проверка: утро вс 20.09 подтверждает длительную 75 мин, строки «⚠️ Длительная урезана» нет.
- **17.09.2026 вечер — инцидент синка Coros** (CHANGELOG, верхняя запись): Track Run `sportType=103` отбрасывался фильтром
  (`SPORT_TYPES_RUN` теперь 100–103), кнопка «здоровье» веб-формы падала на `pending`, окно `since` списка —
  `ACTIVITY_SYNC_LOOKBACK_DAYS` 7 дн (ловит поздние выгрузки и пропущенную 17.09 16:47). Тесты `tests/test_sync_filters.py`.
  Прод-проверка после деплоя app+bot: лог `Coros list_activities: raw=… dropped_sport=…`, тренировка 17.09 в БД, разбор в чате. BACKLOG #356.
- **Ранее — «интенсив ждёт нас с субботы» (#306 ч.2 / #355), 17.09.2026 день**
  (CHANGELOG 17.09, верхняя запись): правила 11/18/19 заякорены к флагнутой тренировке, промпт «граница ≠ план», строка
  «Интенсив — не раньше …» на лёгкой карточке — только при качественном дне в плане (решение владельца). Код в `main`,
  тесты зелёные (1135); **закоммичено `07b3870`, запушено, задеплоено app+bot 17.09** (бэкап `backup_2026-09-17_15-58-53`). Прод-проверка: утро
  Пт 18.09 — без «интенсив с …» и без строки «не раньше» (в плане недели качества нет); `/plan` в вс ставит качество по
  `hard_days_max`, а не по уехавшему сроку. Открыто: #354, #350, остаток #243, #349, #344.
- **Ранее 17.09 — допуск объёма по состоянию + диапазон времени (#352/#353)**
  (`docs/coach/TASK_2026-09-13_athlete_requests.md` §9: инцидент «40 → 30 мин», два решения владельца, что сделано). Код в
  `main`, тесты зелёные (1127); **закоммичено `5cfa63e`, запушено, задеплоено app+bot 17.09** (бэкап
  `backup_2026-09-17_10-47-46`). Прод-проверка — TASK §9
  (пт 18.09 утро: «30–35 мин» без «⚠️ урезан»; вс длительная «64–75 мин»).
- **Ранее тем же вечером — потолок объёма и календарь стартов, #243 ч.1 закрыта 17.09.2026** (закоммичено `f3e304b`, задеплоено)
  (`docs/coach/TASK_2026-09-17_races_and_volume_ceiling.md`: свод литературы, три решения владельца, дизайн, остаток #243 в §6).
  Код в `main`, тесты зелёные (1116), `create_app` ок; **деплой (`app` + `bot`) и коммит — по запросу владельца**
  (см. CHANGELOG 17.09, вторая запись). Прод-проверка после деплоя — TASK §5. Новые BACKLOG: #350 (скорость роста
  объёма — решение владельца), остаток #243 (прогноз результата, тактика, сверка отчёта, приоритет A/B).
- **Предыдущая точка того же дня — частота беговых дней в чате/утре, #347/#348 закрыты 17.09.2026** (закоммичено `9103cc6`, задеплоено)
  (`docs/coach/TASK_2026-09-13_athlete_requests.md` §8: вопрос владельца «оценит ли тренер просьбу о тренировке в
  день отдыха», два решения — мягкий кэп + правило 22). Код в `main`, тесты зелёные (1096), `create_app` ок; **деплой
  (`app` + `bot`) и коммит — по запросу владельца** (см. CHANGELOG 17.09). Новый техдолг — #349 (контекст чата при
  разовом правиле показывает `hard_days_max=0` на неделю). Дальше — по BACKLOG «Приоритеты», как ниже.
- **Предыдущая точка — P0 #338–#343 закрыт 16.09.2026** (`docs/coach/TASK_2026-09-13_athlete_requests.md`
  §7: разбор Fable, три решения владельца, что сделано). Код в `main`, тесты зелёные (1080), **деплой и
  коммит — по запросу владельца** (см. CHANGELOG 16.09). Дальше по BACKLOG «Приоритеты»: #243 ждёт даты старта;
  P3 по решению владельца (#116, #126, #125, #84/#85) или фичи (#255, #309, #248); техдолг #344 (датозависимый
  тест `long_run_hold`, падает в среду на `main`). Тем же днём (#345, закоммичено отдельно): объём недели
  плоский только по стойким сигналам, мезоцикл считает недели роста — ожидание на проде: `/plan` 20.09 → «неделя
  3/4 (рост) · цель ~29 км», не разгрузка до 20.
- Предыдущая задача — `docs/coach/TASK_2026-09-12_status_and_lthr.md`: решения владельца 12.09.2026 выполнены
  полностью (статус подопечного + лестница качественных дней, длительная 40 % до 40 км, рост объёма при интенсиве
  закрытом правилами 16/17, полевой тест ПАНО M3.2). Всё задеплоено, тесты зелёные, закоммичено и запушено 12.09.
- **Порядок работ — `BACKLOG.md`, раздел «Приоритеты»** (P0 безопасность → P1 нагрузка/планы → P2
  данные → P3 приложение). **16.09.2026: P0 #338–#343 закрыт** (потолки объёма в ad-hoc пути, `coach/day_caps.py`).
  Состояние на 11.09.2026: P0 и P1 были закрыты, кроме #243 (ждёт даты
...
| Просьбы подопечного об изменении тренировки — потолки объёма (#338–#343, ✅ 16.09.2026) и частоты (#347/#348, §8, ✅ 17.09.2026) в чате/утре | `docs/coach/TASK_2026-09-13_athlete_requests.md` |
| Потолок недельного объёма и календарь целевых стартов — свод литературы, решения, остаток #243 (✅ ч.1 17.09.2026) | `docs/coach/TASK_2026-09-17_races_and_volume_ceiling.md` |
| Чек-лист 12.09.2026: длительная/объём/тест ПАНО (WP1–WP3, ✅ закрыт) | `docs/coach/TASK_2026-09-12_status_and_lthr.md` |
