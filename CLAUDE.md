...
- **Статусы дорожной карты — только в DEV_PLAN §9** (C0–C9, D0–D8, E0–E3, F0–F7 закрыты к 01.09.2026;
  ...
  ждёт накопления insights), план к цели/гонке #243 (ТЗ зафиксировано, ждёт даты старта), наклон
  базовой линии #259, хвост #289 (поправка `hr_vs_baseline` при detraining), M3.2 полевой тест ПАНО
  (за владельцем).
...
- **Порядок работ — `BACKLOG.md`, раздел «Приоритеты»** (P0 безопасность → P1 нагрузка/планы → P2
  данные → P3 приложение). Состояние на 04.09.2026: P0 и P1 закрыты, кроме #259 (research) и #243
  (ждёт даты); следующий кандидат — #259, затем P2.
