...
- **⏩ ТЕКУЩАЯ ТОЧКА ВОЗОБНОВЛЕНИЯ — допуск объёма по состоянию + диапазон времени (#352/#353), 17.09.2026 поздний вечер**
  ...
  `main`, тесты зелёные (1127), `create_app` ок; **деплой (`app` + `bot`) и коммит — по запросу владельца** (CHANGELOG 17.09,
  третья запись). Прод-проверка — TASK §9. Открыто: #354 (`residual_load` от верха), #350, остаток #243, #349, #344.
