...
- **⏩ ТЕКУЩАЯ ТОЧКА ВОЗОБНОВЛЕНИЯ — план на день после скриншота сна, 19.09.2026** (CHANGELOG, верхняя
  ...
  датозависимый `test_day_caps` — BACKLOG #362, падает и на чистом `main`); **деплой (`app` + `bot`) и
  коммит — по запросу владельца**. Прод-проверка: 9:00 напоминание при отсутствии скрина; скрин в 8:xx →
