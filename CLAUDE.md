...
## Git / коммиты
- **Коммить только по запросу пользователя** (не автоматически). Не в `main` напрямую — рабочая ветка.
- `GITHUB_TOKEN` для push лежит в `.env`; отдельно не спрашивать. Пуш — только когда попросят.
- Каждый коммит — логически завершённая единица; `CHANGELOG.md` в том же коммите.
