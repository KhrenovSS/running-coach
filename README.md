...
- 17.09: допуск объёма дня в чате/утре — по состоянию (15 % без стойких сигналов усталости, 5 % при них, гистерезис < 10 мин / < 1 км), ровный день назначается диапазоном «30–40 мин · ≈4.3–5.7 км» (`coach/volume_range.py`), факт внутри — по плану.
- 17.09: потолок недельного объёма по дистанции цели (5 км 40 · 10 км 50 · ПМ 55 · марафон 65, без стартов 55) и календарь целевых стартов в чате (`coach/races.py`, `race_plan.py`: тейпер 75 %, неделя старта 55 %, старт в плане кодом); `/plan` больше не стирает окно доступности (#351).
- 17.09: частота беговых дней в чате/утре — лишний день сверх `run_days_max` понижается до лёгких ≤ 30 мин (`day_caps.cap_run_days`, мягко, плановые дни не трогает), правило 22 `run_streak` (серия без выходного → только лёгкий); плановый отдых строкой не хранится — «день отдыха» = отсутствие назначения.
- 16.09: объём недели плоский только по стойким сигналам safety (сон одной ночи не держит), мезоцикл 3:1 считает только недели роста (#345)
- 16.09: потолки объёма в чате/утре — просьба «пробежать больше» оценивается кодом (`coach/day_caps.py`, `segment_trim.py` — структурные режутся по гайдам, база сравнения план vs факт `baseline`; #338–#343)
