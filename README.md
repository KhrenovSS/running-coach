...
Система анализирует тренировки из TCX/FIT-файлов (формат не привязан к конкретным часам — подойдут Garmin, Coros, Polar и любые другие) и поддерживает **автоматическую синхронизацию с Coros Training Hub** — достаточно нажать одну кнопку, чтобы подтянуть новые беговые тренировки с часов Coros. Определяет тип бега (интервальная, темповая, восстановительная, длительная), отслеживает пульсовые зоны, набор высоты, погодные условия и динамику веса. В будущем — рекомендации тренировок на основе накопленной статистики.
...
- **Автосинхронизация с Coros** —一键 синхронизация всех беговых тренировок с Coros Training Hub (FIT через неофициальный API); ручная загрузка файлов больше не нужна
- **Загрузка TCX/FIT** — парсинг файлов любых часов (дистанция, пульс, высота, GPS; без привязки к бренду)
...
│   │       └── common.py     # Общая логика (очистка треков, сегментация, погода)
