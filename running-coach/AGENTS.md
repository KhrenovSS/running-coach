...
## Что было сделано (29.06.2026, вечер)
1. **Исправлен баг с дублирующим `reply_markup`** в `_sync_for_user()` (строка 362) — убран дублирующий `reply_markup={"inline_keyboard": rows}`, теперь `reply_markup` передаётся один раз, уведомления о новых тренировках доходят корректно
2. **Проверка данных о сне (Recovery data check)**:
   - Функция `daily_recovery_check_job()` — проверяет наличие `DailyMetrics` за последние 12 часов
   - Старт в 10:00 (через `run_daily`)
   - Если данные о сне **есть** → следующая проверка в **18:00**
   - Если данные **нет** → проверка каждые **2 часа** (12:00, 14:00, 16:00, 18:00)
   - Ночью (0:00–8:00) и после 20:00 — не отправляет уведомления (пользователь спит)
   - При отсутствии данных отправляет напоминание: "🌙 Нет данных о восстановлении — используй /sync"
3. **Уведомление о новой тренировке**: при синхронизации через `/sync` пользователь получает сообщение с деталями тренировки и кнопками оценки (0–10)
