...
## Текущее состояние (Session — 28.06.2026, финальная)
...
**Что сделано в сессии 28.06.2026 (финальная):**
- **Навигация по годам/месяцам**: на главной странице переключатели годов и месяцев (только с данными), показываются все тренировки за выбранный месяц
- **Убрана колонка «Сегм.»** с главной таблицы
- **Coros sync**: фоновый поток + polling прогресса, no more alerts/confirms
- **Логирование**: `src/logger.py` + `/logs` endpoint
- **Починена кнопка Coros Sync**: `syncCoros` в глобальной области, работает при нажатии
- **Удалённые тренировки загружаются заново**: убран `since` из запроса к Coros
