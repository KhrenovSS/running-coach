...
## Текущее состояние (Session — 28.06.2026, вечер)
...
**БД:** SQLite, файл `running_coach.db`, 3 тренировки

**Что сделано в сессии 28.06.2026 (вечер):**
- **Каденс Coros (RPM→SPM)**: авто-удвоение каденса <100 spm, т.к. Coros хранит в RPM
- **Training Effect / VO₂max**: удалены из парсера и отображения — Coros Pace 4 не экспортирует их в FIT (вычисляет в облаке EvoLab)
- **Часовой пояс**: исправлена конвертация UTC→MSK — явное указание UTC перед `astimezone()`, fallback на `Europe/Moscow` если GPS не определён; существующие записи в БД исправлены (+3ч)
- **UI детальной карточки**: переработан формат метрик — метка сверху (жирная, 13px), значение крупно (20px, зелёное), единица снизу (жирная, 12px); подъём/спуск в отдельных карточках; погода вынесена под дату; калории отдельной карточкой
- **UI главной таблицы**: колонки «Длит.»→«Длительность», «Метрики»→«Энергозатраты, ккал» (крайняя правая), выравнивание по центру; набор/спуск как `↑x / ↓y`
- **Удалён отладочный код**: сохранение копий FIT-файлов, логирование session-полей
