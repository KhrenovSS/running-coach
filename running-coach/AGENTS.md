...
## Текущее состояние (Session — 28.06.2026, итоговая)
...
**БД:** SQLite, файл `running_coach.db`, 26 тренировок

**Что сделано за сессию 28.06.2026:**
- **Навигация по годам/месяцам**: переключатели годов и месяцев (только с данными), все тренировки за выбранный период
- **Убрана колонка «Сегм.»** с главной таблицы
- **Coros sync**: фоновый поток + polling прогресса, удалён `since` из запроса, починена кнопка
- **Логирование**: `src/logger.py` + `/logs` endpoint
- **Убраны все alert/confirm**: тренировки сохраняются сразу
- **FIT-парсер**: `fitparse` → `fitdecode` (совместимость с Coros)
- **Шифрование пароля Coros**: `src/crypto.py` (Fernet), ключ в .env, на странице настроек плейсхолдер
- **`.gitignore`**: добавлен `*.log`
- **Комментарии**: добавлены во все функции/классы по требованию стиля; AGENTS.md уточнён — писать сразу
- **Актуализирован AGENTS.md**: правила комментариев ужесточены

**Реализованный функционал:**
- Парсинг TCX (дистанция, пульс, высота, GPS, время)
- Парсинг FIT (дистанция, пульс, каденс, высота, GPS, время, калории)
- Каденс: авто-удвоение для Coros (RPM→SPM)
...
- Погода (Open-Meteo, температура + WMO иконка) — фоновая информация
...
- Синхронизация Coros (аутентификация, список активностей с пагинацией, скачивание FIT)
- Пароль Coros шифруется Fernet-ключом из .env
- Логирование всех операций с ротацией (app.log, /logs endpoint)
