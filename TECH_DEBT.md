...
4. **Спринт 4 — Процессы, интеграции, мульти-брендовая архитектура** (3–4 дня)
   ...
   **Как проверить:**
   ...
   - [x] `grep -rn "coros_email\|coros_password" src/` → 0 совпадений.
...
- [x] **4.5.24** Telegram `/start` → регистрация → вход через веб — работает. *(Фаза 1 плана)*
- [x] **4.5.25** Загрузить TCX-файл через веб → тренировка сохраняется → время на странице в MSK. *(Фаза 1)*
- [x] **4.5.26** Синхронизация Coros через кнопку → тренировки загружаются → время в MSK. *(Фаза 1)*
- [x] **4.5.27** `daily_weight_job` и `daily_recovery_check_job` — в логах нет ошибок (успешно выполнились).
- [x] **4.5.28** `alembic downgrade -1` → `alembic upgrade head` — чистый цикл без ошибок. *(Фаза 1)*

> **⚠️ Урок (02.07.2026):** после применения миграции `c9d8e7f6a0b2` (DROP `coros_email`, `coros_password`, `last_coros_sync` из `users`) контейнер `bot` **не был пересобран** и продолжал работать со старым кодом, где SQLAlchemy-модель `User` содержит эти поля. При любом запросе бот падал с `UndefinedColumn: column users.coros_email does not exist`.
>
> **Вывод:** миграции, удаляющие колонки, требуют обязательной пересборки **всех** контейнеров, которые используют модель. Alembic запускается только в `app`. `bot` использует ту же модель, но alembic в нём не стартует — расхождение версий кода и схемы БД не детектируется автоматически. После любой DROP-миграции: `./bin/docker.sh build bot && ./bin/docker.sh up -d bot`.

**Регрессионные тесты:**
- [x] Старые тренировки (конвертированные в п.8.4) отображаются с правильным временем. *(Фаза 1)*
- [x] Новые тренировки (загруженные после перехода на TIMESTAMPTZ) отображаются с правильным временем. *(Фаза 1)*
- [x] `DeletedTraining.begin_ts` — корректно при удалении и повторной загрузке. *(Фаза 1)*
- [x] `WeightMeasurement.measured_at` — график веса показывает правильные даты. *(Фаза 1)*
   ...
   - [x] **1 новая тренировка загружена и отображается с правильным временем.** *(Фаза 1)*
...
### Обновлённый план дальнейших работ (от 03.07.2026 — Фаза 3Б выполнена)

После завершения Sprint 4.5 и Sprint 4 п.12+14, остаётся выполнить:

#### Фаза 1 — Добить остатки Sprint 4 + Sprint 4.5 (1 день)
- [x] **п.12+14.9**: удалить поля `coros_email`, `coros_password`, `last_coros_sync` из модели `User`. Alembic-миграция: перенос данных в `WatchCredential`.
- [x] Удалить устаревшие упоминания `coros_client.py` из документации.
- [x] **Sprint 4.5 Фаза 7**: проверить Telegram, TCX, Coros sync, `alembic downgrade/upgrade` (4.5.24–4.5.28 + регрессия).

#### 🐛 Критический баг — Потеря тренировок при задержке Coros API

**Приоритет:** 🔴 Исправить перед Фазой 2

**Где:** `src/services/sync_service.py:212`, `src/watch/coros.py:116-126`

**В чём проблема.**
`sync_activities_for_user()` вызывает `client.list_activities(since=cred.last_activity_sync_at)`. 
`CorosWatchClient.list_activities()` фильтрует активности клиентски: `if since_ts and start_ts <= since_ts: continue`.
После синхронизации `last_activity_sync_at` обновляется на `datetime.now(utc)`.

Если Coros API не успел обработать тренировку к моменту синхронизации (задержка 5–30 мин после финиша через Bluetooth→телефон→Coros Cloud), активность не попадёт в список. На следующей синхронизации `since` уже позже `startTime` → тренировка **потеряна навсегда**.

**Пример:**
1. Синхронизация в 14:00, `last_activity_sync_at = 14:00`
2. Тренировка: 14:30–15:00 (startTime = 14:30)
3. Coros Cloud обработал в 15:15
4. Следующая синхронизация в ~15:02 (интервал 60 мин ±20%): `since = 14:00`. Тренировка ещё не готова → не найдена.
5. Следующая синхронизация в ~16:04: `since ≈ 15:02` (обновилось после шага 4). 14:30 < 15:02 → **пропущена навсегда**

**Почему это плохо.** Пользователь регулярно теряет тренировки. Доверие к системе падает. Для модуля аналитики это критично — без полной истории рекомендации будут неверными.

**Как исправить.**
- Добавить lookback-буфер: `since = last_activity_sync_at - timedelta(hours=2)`.
- Защита от дублей уже есть: `existing_begin` (строка 216) проверяет по `begin_ts`, `deleted_trainings` (строка 232) — по совпадению времени.

**Проверка здоровья:**
- [x] Health sync **не** подвержен этому багу: он запрашивает окно 120 дней (`start_day = today - 120d`) и фильтрует по `entry_date in existing_dates` — опоздавшие метрики подхватятся на следующей синхронизации.

**Как проверить исправление:**
- [x] Принудительная синхронизация после тренировки загружает её, даже если Coros обработал активность с задержкой.
- [x] Дубли не создаются (проверка по `begin_ts`).
- [x] `existing_begin` корректно отсеивает уже импортированные тренировки.

---

#### Фаза 3Б ✅ — inline-клавиатура оценки тренировки + отображение в веб (03.07.2026, выполнено)
...
**Что сделано:**
- [x] **3Б.1** `src/services/sync_service.py` — в `sync_activities_for_user()` собираем данные каждой новой тренировки, после `db.commit()` отправляем отдельное сообщение с inline-клавиатурой `0–10`.
- [x] **3Б.2** `src/web/routes/uploads.py` — в три вызова `telegram_notify()` добавлен `reply_markup` с inline-клавиатурой `0–10`.
- [x] **3Б.3** `src/web/routes/pages.py` — на странице `/session/{id}` читаем `TrainingFeedback` и передаём `rating_display` в шаблон. В списке тренировок на главной — `feedback_map` с оценками.
- [x] **3Б.4** `src/web/templates/session.html` — добавлена строка `⭐ Оценка: X/10`.
- [x] **3Б.5** `src/web/templates/index.html` — добавлена колонка `Оценка` в таблицу тренировок.
- [x] **3Б.6** Миграция БД не требуется — таблица `training_feedback` уже существует.

#### Фаза 2 — Sprint 6: Per-user частота синхронизации (бренд-независимая) ✅ (03.07.2026, выполнено)
- [x] **6.1–6.2**: колонки `activity_sync_interval`, `health_sync_interval` в `WatchCredential` + Alembic-миграция (миграция `b6c7d8e9f0a1` уже содержала колонки).
...
- [x] **6.5–6.6**: UI-поля в настройках + `POST /settings`.
- [x] **6.7**: баннер для новых пользователей.
- [x] **6.8**: статус автосинхронизации per-brand на главной (next_run учитывает per-user интервалы).
- [x] **6.9–6.10**: Telegram-бот + CHANGELOG.
...
- [ ] **4.3** Для Polar / Garmin / Suunto / Другие — заглушка: «⚠️ Интеграция с {Brand} пока не реализована. Вы будете получать уведомления, а загружать тренировки — через веб-интерфейс (TCX/FIT).»
- [ ] **4.4** Сохранять выбранный бренд в `WatchCredential.brand` при регистрации.
- [ ] **4.5** Веб-интерфейс: на странице настроек показывать привязанный бренд и кнопку «Отвязать».
- [ ] **4.6** CHANGELOG + документация.

#### Далее — Модуль аналитики (8 этапов из `decision_module_design.md`)
- [ ] **Этап 0** — Каркас и данные (`src/coach/`, таблицы, `config.py`, фикстуры).
- [ ] **Этап 1** — Аналитика (Skills) + State Assessor.
- [ ] **Этап 2** — Движок + безопасность (P1) + Recovery Timing (P2).
...
#### Затем — Sprint 7: Admin panel (отложен)
- [ ] **7.1–7.10**: колонка `role`, `get_admin_user`, дашборд, управление пользователями, аудит.
...
*Версия: 1.5 · Дата: 03.07.2026 · Проект: AI Running Coach*
