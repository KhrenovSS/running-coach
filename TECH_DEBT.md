...
**Проверка здоровья:**
- [ ] Health sync **не** подвержен этому багу: он запрашивает окно 120 дней (`start_day = today - 120d`) и фильтрует по `entry_date in existing_dates` — опоздавшие метрики подхватятся на следующей синхронизации.

**Как проверить исправление:**
- [ ] Принудительная синхронизация после тренировки загружает её, даже если Coros обработал активность с задержкой.
- [ ] Дубли не создаются (проверка по `begin_ts`).
- [ ] `existing_begin` корректно отсеивает уже импортированные тренировки.
