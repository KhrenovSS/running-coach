...
- [ ] **4.5.24** Telegram `/start` → регистрация → вход через веб — работает. *(Фаза 1 плана)*
- [ ] **4.5.25** Загрузить TCX-файл через веб → тренировка сохраняется → время на странице в MSK. *(Фаза 1)*
- [ ] **4.5.26** Синхронизация Coros через кнопку → тренировки загружаются → время в MSK. *(Фаза 1)*
- [x] **4.5.27** `daily_weight_job` и `daily_recovery_check_job` — в логах нет ошибок (успешно выполнились).
- [ ] **4.5.28** `alembic downgrade -1` → `alembic upgrade head` — чистый цикл без ошибок. *(Фаза 1)*

**Регрессионные тесты:**
- [ ] Старые тренировки (конвертированные в п.8.4) отображаются с правильным временем. *(Фаза 1)*
- [ ] Новые тренировки (загруженные после перехода на TIMESTAMPTZ) отображаются с правильным временем. *(Фаза 1)*
- [ ] `DeletedTraining.begin_ts` — корректно при удалении и повторной загрузке. *(Фаза 1)*
- [ ] `WeightMeasurement.measured_at` — график веса показывает правильные даты. *(Фаза 1)*
   ...
   - [ ] **1 новая тренировка загружена и отображается с правильным временем.** *(Фаза 1)*
...
- [ ] **Sprint 4.5 Фаза 7**: проверить Telegram, TCX, Coros sync, `alembic downgrade/upgrade` (4.5.24–4.5.28 + регрессия).
