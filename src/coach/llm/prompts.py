...
- proposal: объект ИЛИ null. Заполняй ТОЛЬКО для нового или ИЗМЕНЁННОГО назначения
  (утро, вопрос «что мне сегодня делать», коррекция). Если сегодняшнее назначение
  уже дано (planned_workout в контексте) и менять его не нужно — null: карточка
  уже у подопечного, не дублируй её. В разговоре «просто поговорить» — null.
  ...
  Другие имена не принимаются. Для workout_type="rest": target_zone = 1,
  duration_min и distance_km = null.
