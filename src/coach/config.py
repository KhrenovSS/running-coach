...
# ускорений сохраняем; качественная работа — уменьшаем повторы (не ниже SEGMENT_WORK_REPEAT_MIN),
# разминку/заминку не трогаем (гайд 44: потолки качества за сессию; гайд 45: режут наполнитель, не
# ключевой стимул). (Trim structured workouts: cut steady minutes / repeats, keep warm-up/cool-down.)
STRIDES_MAX_PER_SESSION = 8
SEGMENT_WARMUP_MIN_MIN = 10        # разминка при урезании не короче
SEGMENT_COOLDOWN_MIN_MIN = 5       # заминка при урезании не короче
SEGMENT_WORK_REPEAT_MIN = 2        # повторов качественной работы при урезании не меньше
...
STRIDE_HOWTO = ("Ускорения: плавный разгон, быстро и свободно, без спринта и натуживания; "
                ...
                "до полного восстановления")  # как выполнять (гайды 46/61), без чисел
...
# гейт «объективный фон в норме»: |z| hr_vs_baseline. 08.09.2026 (#259): σ стала сессионной
# (3.4 вместо км-RMSE 5.5 уд/мин) — порог поднят с 1.0, чтобы гейт остался ≈ 5 уд/мин
RPE_BASELINE_Z_MAX = 1.5
...
# (Adaptive run-day cap: recent max + STEP, clamped to [FLOOR, CAP].)
PLAN_RUN_DAYS_CAP = 6
...
# «Интенсив — не раньше …» на карточке НЕхардового дня показываем, только если до срока не меньше
# этого (#306, 07.09.2026: строка на лёгкой при остатке 38 мин — шум). Сам срок — от начала
# последней тренировки + часы восстановления, не «сейчас + остаток» (не дрейфует).
