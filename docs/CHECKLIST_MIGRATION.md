...
## Идемпотентность (Idempotency) — рекомендация

> В существующих миграциях (25 файлов) не применяется: Alembic ведёт версию сам, повторный запуск
> не случается. Полезно только для ручных `op.execute`/индексов, создаваемых вне цепочки.
