...
- ⬜ **E2 — дистилляция реальных книг**: по одной книге → ревью конспектов
  (агент+владелец: без цитат, key_rules адекватны, темы не дублируют существующие) →
  перенос в `src/coach/knowledge/guides/` → пересмотр WEEKLY_PROMPT под plan-гайды.
  Гвард: key_rules_digest ≤ ~50 строк (кэш system[0]). **Ждёт книг от владельца.**
...
2. **Гигиена**: #251 (fallback-карточки не жгут LLM-бюджет), #247 (numeric-checker
   проза↔карточка), #242 (гайды методики инлайном в разбор/отчёт), #256 (атомарный reclaim).
