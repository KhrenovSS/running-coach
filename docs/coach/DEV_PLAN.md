...
- **Утренний вердикт — по скриншоту сна** (19.09.2026): скрин за сегодня → вердикт сразу;
  **09:30 — резерв** для дней без скрина (дедуп — заявка `coach/morning_state.py`),
  напоминание о скрине — 09:00. Поздний скрин → один пересчёт с учётом сна.
...
назначением (`orchestrator.handle_chat` kind="morning" → `morning_verdict()`; реализация — `chat_flow.py`), не generic-«базовый
...
- ✅ **E3 — чанки методики инлайном (#242, 25.08.2026)**: `_build_extras` +=
  ...
  (лимит 400 строк orchestrator; 11.09.2026 файл разнесён повторно — чат/утро в `chat_flow.py`, #329).
...
- ✅ **F3 — M2.1 разбора (HRR, 01.09.2026)**: `src/analysis/intervals.py` — границы из
  ...
  Заодно #270: `workout_insights.py` 506→324 строк (11.09.2026 — ещё раз, `workout_insights_context.py`, #329; `insights_baseline.py`,
...
- ✅ **Техдолг**: датонезависимые тесты (#328/#332), гигиена тестов и CI на Python 3.13 (#233/#330),
  **разнос файлов > 400 строк (#329)** без изменения поведения: `planning.py` → `planning_rows.py`
  (строки плана, `week_plan_review`, `confirm_or_adjust_morning`) + `planning_availability.py` (доступность,
  `cancel_days`/`reopen_days`, `blocked_by_unavailable`); `orchestrator.py` → `chat_flow.py` (`ChatReply`,
  `_llm_chat_turn`, `handle_chat`, `morning_verdict`, инициатива); `services/workout_insights.py` →
  `workout_insights_context.py` (план дня, история, RPE, `apply_type_resolution`); `analysis/utils.py` →
  `pace_series.py`. Старые имена реэкспортируются — тесты и monkeypatch не менялись.
- ✅ **#331** — прошедший день отдыха в `/week` — нейтральная строка (было «✗ Отдых · пропущен»).
- ✅ **#247 v2** (решение владельца) — предложение прозы с чужим числом вырезается (`trim_mismatched_prose`,
  `meta.numeric_trimmed`, выключатель `COACH_NUMERIC_TRIM_PROSE`); основание — 0 расхождений из 83 ответов
  за 29.08–11.09 на проде.
- ✅ **#237** (пересмотр решения 06.08) — после принудительного автоподнятия max_hr тренировки батча
  пересчитываются до разбора коучем (`hr_max.reanalyze_batch_after_raise`, `reanalyze_training(check_max_hr=False)`).
- ✅ Мелочи: #236 (`/delete_me` отвязывает чат), #327 (`interval_*` профиля в live-синке/загрузке —
  `analysis/user_params.py`), #274 (константы GPS-очистки), #252 (закрыт как невоспроизводимый), #333 (гвард
  «181 → 181»), #239 (валидация max_hr в `/settings`); аудит 07.2026 — 6 устаревших строк закрыты, 6 багов
  починены (#78 sniff содержимого загрузки, #114, #120, #106, #127, #124). Всё задеплоено (app+bot) 11.09.
...
2. **Гигиена**: ✅ закрыта 29.08.2026 — #251, #247 (v1 детект; v2 — обрезание предложения, 11.09.2026, §11.5), #242, #256, #258 (история без мимикрии).
...
- Недельный план ПЕРСИСТИТСЯ построчно (29.08.2026, `src/coach/weekly_plan.py`):
  ...
  прошедшие дни — факт связанной тренировки (✓) или пропуск (✗), кроме дня отдыха — нейтральная строка без ✓/✗
  (#331, 11.09.2026); план не «меняется» задним числом.
