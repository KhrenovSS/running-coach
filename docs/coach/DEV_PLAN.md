...
- ⬜ **F0 — фундамент корректности**: фиксы аудита, искажающие вход всего дальнейшего —
  #277 (перекос avg_pace: выброшенная дистанция при оставленном времени), #279 (паузы
  в зонах + склейка Z4-отрезков при HR-дропауте), #281 (двойное усреднение зон в 80/20 и
  контексте LLM), #283 (хвостовой км полным весом); заодно мелкие #278/#280/#282.
  Проверка: тесты аудита; №37 после reanalyze ≈ 7.61 мин/км (сейчас 8.06).
- ⬜ **F1 — парсер FIT v2**: lap-сообщения → `laps_json`; timer-события (паузы);
  session-эталоны часов → `device_summary` JSON (дистанция/время/total_strides/min_hr/
  avg_power/динамика/Effort Pace); каналы power/stance_time/vertical_oscillation/
  step_length → опциональные ключи трекпоинтов (только при наличии значения).
  ⚠️ data-safety §5/§7: additive-миграция + db-safety-reviewer; история перечитывается
  существующим `reanalyze` (raw-файлы есть с 13.05.2026). Проверка: №33 — лапы совпали
  с сегментами; замерить рост trackpoints_json.
- ⬜ **F2 — moving-time**: из timer-событий F1 (fallback — gap-эвристика по образцу
  `DRIFT_MAX_SAMPLE_GAP_SEC`); `avg_pace` от moving; кросс-чек с session-эталоном
  (расхождение >5% → флаг качества данных). Закрывает elapsed/moving-часть #284 честно.
- ⬜ **F3 — M2.1 разбора (HRR)**: `src/analysis/intervals.py` по METRICS_GUIDE §5
  (границы фаз — из лапов F1, fallback осцилляции; `hrr60`, `min_hr_recovery`, тренд
  пиков; флаг `poor_interval_recovery` → safety §7). Проверка: `d70e83ac` (7 повторов),
  `511724f0` (10×1000).
...
- ⬜ **F5 — M4.1 структура недели** (METRICS_GUIDE §11): ≤3 качественных, лёгкие дни
  между ними, восстановление после гонки 1 день/3 км; флаги → safety.
- ⬜ **F6 — M4.2–4.4**: downhill-нагрузка (колено), детренированность после пауз
  (связка с F4/VDOT), session-RPE + индекс самооценки (без новых вопросов пользователю).
- ⬜ **F7 — регресс-валидация данных**: при reanalyze авто-сверка per_km с авто-лапами
  часов (>2% → лог/флаг); сверка своего GAP с Coros Effort Pace (телеметрия, не пользователю).

Зависимости: F0 → F1 → (F2 ∥ F3 ∥ F7) → F4 → (F5 ∥ F6).
