...
при закрытом интенсиве `apply_safety_to_targets` держит `target_km = prev_week_km` — с 12.09 только при
правилах усталости/здоровья, с 16.09 только при **стойких** сигналах (`VOLUME_TRANSIENT_SAFETY_RULES`:
распределение нагрузки + сон одной ночи + серия тяжёлых объём не держат). Мезоцикл 3:1 (16.09.2026) считает
только недели роста: `advance_mesocycle` пишет `week_plan.grew`, плоская неделя счётчик не двигает.
