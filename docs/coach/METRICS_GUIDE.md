...
| Жара / холод | `heat_block` | `heat{temp_c, heat_flag, cold_flag, expected_hr_shift_bpm, temp_source, watch_temp_c, watch_bias_c, bias_source, watch_minus_weather_c}` — t: для тренировок ≥ `WEATHER_AVG_MIN_DURATION_MIN` (60) среднее часовых Open-Meteo за интервал бега (#300), иначе значение на старте; нет погоды → датчик часов минус **адаптивная поправка** (зима 08.09.2026: медиана (часы − погода) последних `WATCH_BIAS_MIN…MAX_SESSIONS` 5…10 тренировок за 45 дн, `services/watch_temp_bias.py`; без истории — `WATCH_TEMP_BIAS_C` 3.3; на проде расхождение растёт с холодом: 0…2 °C при +25…30, 5…7 при +10…15, под куркой зимой больше) с гвардом ложной жары (`heat_flag` по часам только если недавняя погода ≥ порога − `WATCH_HEAT_CONFIRM_MARGIN_C`); `cold_flag` при t ≤ `COLD_TEMP_THRESHOLD_C` 0 — только по погоде (датчик под одеждой мороз не покажет) → контекст-флаг `cold` (как `heat`, в enum assessment его нет) + запрос к гайду 49 (мороз/ветер/снег/гололёд); сдвиг `HEAT_HR_BPM_PER_C·(t−HEAT_REF_TEMP_C)` входит в ожидание `hr_vs_baseline` (`temp_shift_bpm`); t зажата в 10–30 °C (диапазон данных, мороз ≠ −18 уд/мин — пересмотр по зимним данным #301, для него копятся `watch_minus_weather_c`); исследование 02.09.2026: +0.5 уд/мин/°C, ~7 уд/мин между <16 и ≥24 °C на равном GAP-темпе |
...
| Набор/спуск высоты (#253, 08.09.2026) | `analysis/utils.calc_elevation` — гистерезис `ELEV_HYSTERESIS_M` 2 м по сырой forward-fill высоте (разворот засчитывается при уходе от экстремума на ≥ порога, хвост — только ≥ порога), как считают часы; замер на 39 тренировках против `total_ascent_m` часов: наивная сумма ×1.22, сглаживание `gap.smoothed_gain_loss` ×1.16 (занижает холмы), гистерезис ×1.03 (p10 0.92, p90 1.14). Сегменты — тем же алгоритмом по своим точкам, сумма сегментов может отличаться от итога на 1–3 м (разворот на границе). Кросс-чек `device_check.ascent_diff_pct`/`ascent_mismatch` (`DEVICE_ELEV_MISMATCH_PCT` 25 %, телеметрия без флага). Флаги `hilly`/`downhill_load_high` и GAP — отдельный сглаженный путь `gap.py`, не затронуты | `TrainingSession.elevation_gain/loss`, `segments_json[].elevation_*` (web «Набор», `get_workout_detail`) |
| Сегменты тренировки (#302, 08.09.2026) | `analysis/segment_laps.lap_segments` → `process_trackpoints(laps=)`: если лапы часов структурные (`intervals.structural_laps`: ≥ 3 лапов и не все непоследние 900–1100 м — программа на часах или ручные отсечки), сегменты = лапы как есть (дистанция/время из лапа — часы надёжнее GPS; HR/каденс/высота — из среза трекпоинтов; `distance_km` 2 dp; ключи `source="laps"`, `lap`, `intensity`), для любого типа тренировки — км-override их не затирает; авто-км лапы и TCX без лапов — прежняя темповая эвристика `segment_by_pace` / км-блоки. Классификация от сегментов не зависит (только `len`): 7×18 с ускорений в лёгком дне остаются `easy` (решение владельца 06.09). `_lap_row` теперь несёт `end_time`/`trigger`/`intensity`/`wkt_step_index`. Провенанс — `computed.inputs.segmentation_source` | `segments_json[]` (web «Сегменты», `get_workout_detail` ≤ 32 строк) |
...
- **≥ 1–2 лёгких дня между качественными** (Дэниелс: минимум 1, в тяжёлых фазах 2;
  ...
  `consecutive_hard_days ≥ 4` — порог из литературы жёстче. С 17.09.2026 — правило 22 `run_streak`:
  `consecutive_run_days ≥ SAFETY_RUN_STREAK_MAX_DAYS` (3; гайд 61 «без 3 тренировок подряд», гайд 47 «день отдыха
  в неделе — всегда») → только лёгкий день; лишний беговой день сверх `run_days_max` в чате/утре понижает
  `day_caps.cap_run_days` (мягко: лёгкие ≤ 30 мин, плановые дни не трогает).
