...
**Статус:** ✅ закрыто 16.09.2026 — разбор Fable подтвердил факты §1–§3 (уточнения — §7.1), решения владельца
приняты 16.09 (§7.2), код/тесты/документация — §7.3. Пункты BACKLOG — #338–#343 (в архиве).
...
## §7. Решение (16.09.2026) — разбор Fable, решения владельца, что сделано

### §7.1. Проверка фактов §1–§3 — верны по существу; уточнения

- Ссылки на строки: `_SCALABLE_TYPES` — `planning_safety.py:240` (фильтр «без сегментов» — `:264-266`, не
  `:289-292`); границы схемы — `llm/schemas.py:41-42`; `week_targets` — `planning.py:97-243`.
- §3.3 занижен: сумма сегментов после clamp не просто «может вырасти», а **откатывала уже применённый
  потолок** `max_duration_min` (clamp 72 → 40, затем `prescriber.py:176` писал назад 72) — правила 9 (боль)
  и 15 (сон < 5 ч) для структурных тренировок не работали.
- У `cap_long_run` был второй обход: `or not proposal.duration_min` — длительная только с `distance_km`.
- «Единственный конструктор `Prescription`» — на уровне файла: `safety.rehydrate` тоже собирает
  `Prescription` (read-only, `week_view`).

### §7.2. Первопричина и решения владельца

В коуче две семьи границ: **safety** (состояние на день: HRV, боль, сон, флаги; `p1_safety` → `clamp`) и
**прогрессия нагрузки** (история объёма: +10 %/нед, доля длительной, потолок минут; `week_targets`,
`cap_long_run`, `cap_week_volume`). Ad-hoc путь видел только первую. Объём — это прогрессия, а не состояние
(`signals` не содержат километров), поэтому «правило 22 в safety» дублировало бы `week_targets`; правильное
место — шаг потолков после `finalize`, симметричный двухпроходной схеме `weekly_plan.py` (finalize → кэп →
повторный finalize). Инвариант DEV_PLAN §1.2 сохранён. Тот же шаг закрыл #342 по первопричине: чат относил
«длительную» к жёстким 60 мин, план — к `min(60, long_run_min_hint)`; порог теперь общий.

Решения владельца 16.09.2026:
1. **Структурная тренировка выше потолка — урезать по литературе**, не снимать структуру и не отклонять:
   ускорения — «символический объём, не в счёт км» (гайд 61), ≤ 8 за тренировку (гайд 46) → режется ровная
   часть, ускорения сохраняются (ровной части < 30 мин → снимаются); качественная работа → повторы вниз,
   разминка/заминка целы (гайд 44: потолки качества за сессию; гайд 45: наполнитель режут раньше ключевого
   стимула); не помещается → структура снимается.
2. **Утро применяет потолки и к plan-строке**, и к предложению LLM (сверка со свежими числами недели).
3. **Флаг «перевыполнил план» — к исходному плану недели**: по максимуму отношений к согласованному и к
   исходному; согласие в чате сигнал дрейфа не гасит.

### §7.3. Что сделано (код, тесты, документация — CHANGELOG 16.09.2026)

- `src/coach/day_caps.py` — `day_targets` / `context_block` (#339) / `cap_day_volume` / `finalize_with_caps`
  (#338); подключено в `chat_flow._llm_chat_turn` (чат и утро) и `planning_rows.confirm_or_adjust_morning`
  (`targets=`, четвёртый элемент — заметки над карточкой).
- `src/coach/segment_trim.py` — `trim_segments` / `shrink_proposal`; константы `STRIDES_MAX_PER_SESSION`,
  `SEGMENT_WARMUP_MIN_MIN`, `SEGMENT_COOLDOWN_MIN_MIN`, `SEGMENT_WORK_REPEAT_MIN` в `coach/config.py`.
- `planning_safety.cap_long_run` — по содержимому (ярлык не спасает), дистанция без минут, структурные через
  `segment_trim`, `race` не режется (#340); `cap_week_volume` — структурные дни ужимаются последними.
- `prescriber.finalize` — сегменты не откатывают `max_duration_min` (#343); порог длительной общий (#342).
- `_plan_for_session.baseline` + `plan_vs_actual.baseline` + `week_plan_review.changed_in_chat`,
  `INSIGHTS_SCHEMA_VERSION` 12 (#341). `SAFETY_CONTRACT` — абзац об объёме.
- Тесты: `tests/coach/test_day_caps.py` (17, включая e2e-репродукцию §1), правки `test_planning.py` /
  `test_weekly_plan.py`, `test_session_metrics.py`, `test_coach_config.py`. Всего 1080 зелёных; #344 — датозависимый
  тест `long_run_hold`, падает на `main` независимо от этих правок.

Как проверить на проде после деплоя: «напомни план на сегодня» → нет строки «Изменил план»; «хочу сегодня
15 км» → карточка в пределах `long_run_km_max`, строка «⚠️ … урезана …»; SQL §6 — `proposal_json.rationale`
с «урезано кодом», `code_trimmed = true`.
