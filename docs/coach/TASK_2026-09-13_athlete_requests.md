...
**Решение владельца 13.09.2026:** когда лимиты обнулятся — это первое, с чего начинается работа;
полный анализ и проектирование ведёт Fable. Пункты BACKLOG — #338–#343.

> Документ — рабочая точка входа: в нём собраны факты инцидента, карта ограничителей и готовые
> read-only SQL, чтобы следующая сессия не повторяла разведку.
...
## §5. Направления решения (черновик — финальное проектирование за Fable)

Не предрешать; ниже — то, что видно из кода, чтобы не начинать с нуля.
