...
## §8. Частота беговых дней (17.09.2026) — продолжение симметрии

**Вопрос владельца:** «Если я попрошу поставить тренировку на день, который уже назначен днём отдыха, тренер не будет
просто соглашаться, а оценит рациональность — объёмы, восстановление — и решит, стоит ли, и какую?»

**Проверка по коду:** объём и интенсивность — да (§7 и 22 правила safety работают в любом пути), **частота — нет**:

- плановый отдых строкой не хранится (`weekly_plan._clean_days:72-73`, «пропуск дня = отдых») — «сегодня по плану
  отдых» = отсутствие строки; гвард `blocked_by_unavailable` (`chat_flow.py:196-206`) видит только дни, отменённые
  подопечным (`UNAVAILABLE_RATIONALE`), и болезнь;
- `run_days_max` / `rest_days_min` / `remaining_run_days_max` считались в `week_targets`, но enforce'ились только в
  `/plan` (`planning.enforce_run_days` ← `weekly_plan.py:257`) и в `day_caps._CONTEXT_KEYS` не входили;
- safety не видел серии беговых дней: `consecutive_hard_days` (правило 7) — только качественные, `trained_days_7d` —
  лишь в паре с монотонностью > 2.0 (правило 20).

Итог до правок: седьмая лёгкая подряд в день планового отдыха проходила без единого детерминированного возражения,
если метрики в норме и в неделе оставался нераспределённый километраж (тогда — урезание минут по остатку км).

**Решения владельца 17.09.2026:** (1) кэп **мягкий** — лишний день не отклоняется, а понижается до лёгкой ≤ 30 мин с
пометкой; (2) **правило 22** в safety на серию беговых дней без выходного. Разработка — на Fable.

**Сделано (#347/#348, CHANGELOG 17.09.2026):**

- `day_caps.cap_run_days` — первый кэп в `finalize_with_caps` (чат и утро): беговые дни недели исчерпаны
  (`run_days_used` = |даты факта `week_targets.done_dates` ∪ даты действующих беговых строк| без целевого дня — день с
  пробежкой и живой строкой не удваивается) и день не из плана недели (`PLAN_STATUSES`; `proposed` из чата день не
  освобождает — повторная просьба кэп не обходит) → `easy`, зона ≤ 2, без структуры/темпа, ≤ `PLAN_EASY_MIN_MINUTES`;
  заметка «⚠️ Беговые дни недели исчерпаны (N из M) — вместо … оставил лёгкие 30 мин». Плановые дни — каркас недели,
  их защищает `enforce_run_days` (как `_KEEP_TYPES`), кэп частоты их не трогает.
- Контекст модели: `run_days_max`, `rest_days_min`, `done_runs`, `run_days_used` + `frequency_rule`; абзац о частоте в
  `SAFETY_CONTRACT`; `PLAN_PROMPT` — качество не на (N+1)-й беговой день подряд (иначе утро срежет правилом 22).
- Правило 22 `run_streak`: `consecutive_run_days ≥ SAFETY_RUN_STREAK_MAX_DAYS` (3) → Z2 без интенсива; сигнал —
  `state._week_signals` по локальным датам назад от сегодня (сегодня без пробежки — от вчера); `project_state`
  обнуляет его для дней плана > 0; в `VOLUME_TRANSIENT_SAFETY_RULES`.
- Тесты — `test_day_caps.py` (+8, включая e2e и «повтор просьбы»), `test_safety_week_rules.py` (+3), `test_state.py`,
  `test_planning.py`, `test_coach_config.py`; всего 1096.

**Что осталось за кодом сознательно:** решение «бежать ли вообще» в свободный день при не исчерпанных беговых днях —
за LLM по числам контекста (кэп даёт гарантию только сверху); порог 3 дня подряд — выбор владельца, одна константа.
Техдолг — BACKLOG #349 (контекст чата при разовом правиле показывает `hard_days_max=0` на неделю).

**Проверка на проде после деплоя (`app` + `bot`):** в день без строки плана при занятых беговых днях написать «хочу
сегодня побегать» → карточка лёгкие ≤ 30 мин + «⚠️ Беговые дни недели исчерпаны (N из M)»; в `recommendations` новая
строка `proposed` с `proposal_json.code_trimmed = true` и следом «лишний беговой день» в `rationale`. При свободном дне —
обычная карточка без заметки. SQL — §6.

---

## §9. Допуск по состоянию и диапазон времени (17.09.2026, вечер)

**Инцидент (прод, user_id = 2):** утро чт 17.09 урезало плановый лёгкий день 40 → 30 мин: «Изменил план … ⚠️ Объём
урезан до 30 мин (≈4.3 км): на неделе осталось 18.2 км, из них 15.7 уже назначено на другие дни».

**По БД:** план вс 13.09 — цель 26.3 (плоская, хвост #345), Вт 37 · Чт 40 · Пт 35 · Вс 75; пн утро добавило лёгкие 35
(пятый беговой день; кэпов в проде ещё не было), вт факт 6.0 (план 5.3). Чт `day_targets` пересчитал неделю свежими
числами: цель 28.9 (правил усталости нет), сделано 10.6, осталось 18.2, на Пт+Вс 15.7 → допуск 18.2 × 1.05 − 15.7 =
3.4 км, плановые 5.7 км → пол 30 мин. Итог недели 30.6 км (+16 %), без урезания 32.0 (+22 %). Safety: `poor_interval_recovery`,
`easy_runs_too_hard`, `downhill_load` — распределение, усталости/здоровья нет.

**Оценка:** 1.4 км лёгкого не критично (шум маршрута ±0.3 км/день; «10 %» — потолок, не норма); причина превышения —
лишний день + длиннее вторник (объём и частота выросли разом), код придерживает по делу, но инструмент грубый: режет
плановый день ради точности ±5 %, которой у модели нет, при чистом вердикте; план и утро смотрели на разные цели (26.3 vs 28.9).

**Решения владельца 17.09.2026:** (1) допуск объёма недели в чате/утре — по состоянию: без стойких правил усталости/
здоровья 15 % + гистерезис (< 10 мин / < 1 км не режем), при них — строго 5 %; `/plan` — 5 %; (2) задание ровного дня —
диапазоном «минимум ради эффекта — план»: «30–40 мин · пульс до 138 · ≈4.3–5.7 км @7:03», низ = max(30, 75 % верха),
длительная — 85 % и не ниже порога длительной; качественные/структурные/race — точка; факт внутри = по плану. Пример
владельца «7–10 км при 30–40 мин» несогласован: км — следствие минут × темп.

**Сделано (#352/#353, CHANGELOG 17.09, третья запись):** `planning_safety.fatigue_signals` (стойкие = `triggered` −
`VOLUME_TRANSIENT_SAFETY_RULES`), `day_caps.day_targets` → `day_volume_tolerance_pct`/`_reason`, `cap_day_volume` с
допуском по состоянию и гистерезисом по эффективному срезу (`DAY_CAP_MIN_CUT_MIN` 10, `DAY_CAP_MIN_CUT_KM` 1.0);
`coach/volume_range.py` (`duration_low`, `minutes_label`, `km_range`, `km_label`), низ считается в `prescriber.finalize`
единожды по итоговому верху (двухпроходные кэпы пересчитывают), карточки день/короткая/неделя/«было: …», `numeric_check`
интервалами, `plan_vs_actual.within_range` + `volume_ratio` 1.0 внутри (`volume_ratio_raw`), `week_plan_review` по
верху/дистанции, `OUTPUT_CONTRACT`/`SAFETY_CONTRACT`. `cap_long_run` строгий — 13.09 по-прежнему 58 мин. Тесты:
`test_volume_range.py` (11); всего 1127.

**Прод-проверка:** пт 18.09 утро — плановые 35 мин без «⚠️ Объём урезан», карточка «30–35 мин»; вс длительная 75 →
«64–75 мин · ≈9.1–10.7 км»; после факта внутри диапазона `workout_insights.computed_json->'plan_vs_actual'` содержит
`within_range: true`, `volume_ratio: 1.0`. Открыто — BACKLOG #354.
