# BACKLOG — закрытые пункты (архив)

> Перенесено 02.09.2026 из `BACKLOG.md`: все строки со статусом ✅, по исходным разделам. Не ведётся.
> Номер #139 был использован дважды — второй экземпляр здесь как #139b.
...
| # | Тег | Описание | Файл / Источник | Статус |
|---|-----|----------|-----------------|--------|
| 139b | [Memory] | **`_weather_cache` без LRU/TTL** — бесконтрольный рост словаря при длительном аптайме. При 1000+ уникальных (lat,lon,date) — утечка памяти. Нужен `functools.lru_cache` или `cachetools.TTLCache`. | `src/parsers/weather.py:7-46` | ✅ Sprint 20b (DEBT-01) |
...
## Закрыто рефакторингом документации (02.09.2026)

| # | Тег | Описание | Файл / Источник | Статус |
|---|-----|----------|-----------------|--------|
| 164 | [Docs] | **Документация не соответствовала проекту** — частично fixed (TESTING.md, API_ROUTES_GUIDE.md, ARCHITECTURE.md, AGENTS.md, CHECKLIST_API.md). Остались замечания #189–#197. | `docs/*`, `AGENTS.md`, `README.md` | ✅ 02.09.2026 — рефакторинг документации (6 коммитов): сироты удалены, дубли слиты, архив, тест `test_docs_links.py`; #189–#197 закрыты ранее |
| 290 | [Analysis] | **Классификатор ставит `tempo` лёгким пробежкам у границы Z2/Z3**: 31.08 (avg HR 137, 38 мин) и 01.09 (avg HR 140, 46 мин, план — лёгкий со страйдами) при потолке Z2 = 138 → LLM в чате 02.09 пересказал «два насыщенных дня подряд (вторник-темповый и понедельничный)». Смотреть гейт `is_quality_session`/порог классификатора относительно LTHR, страйды не должны тянуть тип. | `src/analysis/`, `src/services/insights_baseline.py` | ✅ 04.09.2026 — резолвер ярлыка по плану (`analysis/type_resolution.py`), catch-all tempo устранён |
| 249 | [Config] | Привести recovery-шкалу к Coros §12 (20/70/90): сейчас `RECOVERY_PCT_MODERATE=30` — исторический порог display-слоя. | `src/coach/config.py`, `src/services/recovery_view.py` | ✅ 04.09.2026 — шкала 20/70/90 в config + правило 4 (Fatigued без интенсива) + recovery_view |
| 238 | [Идея] | **Эвристика «ранний пик на низком темпе = глюк датчика»** для адаптивного max_hr: пик в первые минуты пробежки при темпе ниже исторического easy — вопросы к датчику (cadence-lock оптики даёт УСТОЙЧИВЫЙ ложный пульс, медиана его не ловит). v1 закрывает это правилом повторяемости (≥3 превышений за 30д); вернуться, если появятся ложные предупреждения. | `src/services/hr_max.py`, `src/analysis/utils.py` | ✅ 04.09.2026 — analysis.utils.early_peak_suspect, флаг hr_early_peak, пик без первого окна |
| 307 | [Test] | **`test_poor_interval_recovery_delays_next_hard` зависит от реального времени**: `evaluate_safety` получает фиксированный `now=01.09.2026`, а `clamp(AGGRESSIVE, verdict, state)` — без `now` → с 03.09 12:00 UTC `earliest_next_hard` уже в прошлом, интервалы не режутся, `clamped is False`. Передать тот же `now` в `clamp`. Красный на main с 03.09 (замечено при работе над отменой дней плана). | `tests/coach/test_safety_clamp.py:341-356` | ✅ 04.09.2026 — clamp получает тот же now, что вердикт |
| 308 | [Coach] | **Монотонность и страйн недели (Фостер)**: monotony = средняя дневная нагрузка / SD, strain = недельная нагрузка × monotony — по session-RPE (`computed.session_rpe.load_au`) или баллам нагрузки; литература рекомендует, в коде нет. Добавить в `week_report` как concern при monotony > 2. | `src/coach/week_report.py`, `docs/coach/METRICS_GUIDE.md §12` | ✅ 04.09.2026 — coach/load_monotony.py: week_report concern + правило 20 safety |
| 220 | [Analytics] | `weekly_volume` бакетит недели по UTC, игнорируя `TrainingSession.timezone` — off-by-one для не-UTC. | `src/services/repositories.py` (weekly_volume) | ✅ 04.09.2026 — planning_window.local_week_volumes в week_targets |
| 292 | [Coach] | **`confirm_or_adjust_morning` подтверждает не ту строку**: утро 02.09 пометило `confirmed` строку `planned` id 31 (tempo), хотя план дня накануне заменён строкой `proposed` id 34 (easy) — `PLAN_STATUSES` исключает `proposed`, а `unchanged_today` сравнивает с любой последней строкой даты. План-vs-факт недели (`week_plan_review`) увидит «темповую выполнена». Решить: включать `proposed` на ту же дату в выбор plan_row или помечать вытесненные строки. | `src/coach/planning.py` | ✅ 04.09.2026 — confirm_or_adjust_morning берёт последнюю действующую строку дня |
| 294 | [Coach] | **Окно доступности не персистится**: «могу бегать пн–чт» живёт только в истории чата (8 ходов) — на следующем `/plan` модель его уже не увидит. Хранить в `UserModel.params_json.week_plan.availability` (merge-паттерн `advance_mesocycle`), отдавать в `week_targets`/PLAN_PROMPT, задавать из чата (`log_suggestion`-подобный тап) или командой. | `src/coach/planning.py`, `src/coach/llm/prompts.py` | ✅ 04.09.2026 — available_weekdays → params_json.week_plan.availability, фильтр days_ahead_allowed, отмены переживают /plan |
| 295 | [Coach] | **План хранит `max_zone`, а не уд/мин** — потолок пульса карточки считается при рендере из текущего якоря зон; смена якоря (max_hr → LTHR, F4 01.09) молча меняет числа будущих дней (Z2: 144 → 138), владелец читает это как «план изменили». Сегменты уже хранят `hr_ceiling`. Рассмотреть фиксацию `hr_ceiling` в `target_json` при `finalize` + правило обновления (утреннее подтверждение re-clamp'ит) и явную пометку «зоны пересчитаны от ПАНО». | `src/coach/prescriber.py`, `src/coach/render.py` | ✅ 04.09.2026 — target.hr_ceiling фиксируется в finalize, render читает его |
| 305 | [Fix] | **Утро подтверждает устаревшую строку плана** (тот же корень, что #292 — решать вместе): `confirm_or_adjust_morning` берёт `plan_row` только из `PLAN_STATUSES`, а `unchanged_today` сравнивает с последней строкой любого статуса → когда чат уже дал новое назначение на сегодня (03.09: #32 easy 30 мин получил `confirmed`, карточка — по #36 long 60). На показ не влияет (week_view берёт последнюю строку на дату), статусы в БД врут. | `src/coach/planning.py:confirm_or_adjust_morning`, `src/coach/turn_context.py:unchanged_today` | ✅ 04.09.2026 — вместе с #292 |
| 311 | [Coach] | **Проза недельного отчёта расходится с карточкой по качеству**: 04.09 отчёт написал «два интенсивных дня подряд», карточка — «Качество: 0» (по пульсу ни одна `tempo` не дотянула до `is_quality_session`). Модель берёт ярлыки `tempo` из `recent_workouts`. Промпт: интенсивность недели — только по `week_report.this.quality_runs`/`hard_time_share`; либо в `recent_workouts` отдавать `is_quality` вместо сырого ярлыка (#290). | `src/coach/llm/prompts.py:WEEKLY_PROMPT`, `src/coach/tools/history_tools.py` | ✅ 04.09.2026 — WEEKLY_PROMPT: интенсивность по is_quality/hard_time_share |
| 263 | [Coach] | **Расширить выборку км-точек темповыми сегментами** — `_collect_window_points` берёт только steady-типы (easy/long/recovery), на темповом диапазоне точек в полосе ±0.25 мин/км часто <5 → прогноз «пульс на темпе» (`expected_hr_at_pace`) будет часто None («мало данных»). Кандидаты: км-точки tempo/interval-сессий из ровных сегментов. | `src/services/workout_insights.py:_collect_window_points` | ✅ 04.09.2026 — _collect_window_points(include_quality) для полос |
| 264 | [Coach] | **Ориентир темпа/дистанции в карточке пропадает для recovery/Z1 и Z3+** (инцидент 27.08.2026: восстановительный бег без строки ориентира — владелец выбирает по ней маршрут). Причина: `pace_at_hr_band` требует ≥5 км-точек в односторонней полосе `[потолок−10, потолок]`, а пульс сосредоточен в узком коридоре (для Z1 потолок 125 → 2 точки; Z2 → 24, работает). Решение — ступенчатая деградация A (узкая полоса) → B (широкая ±25 + локальная поправка наклоном, гейт Δ≤15) → C (медиана `avg_pace` сессий типа) → D, с `quality` в `predicted` и честной пометкой «прикидка» в карточке. **Деградированные оценки не должны попадать в safety-clamp** (`_pace_clamp_context` — только уровень A). Полное задание с диагностикой, формулами, проверкой на живых данных и тестами: `docs/coach/TASK_pace_estimate_fallback.md`. Родственные: #259 (занижённый наклон OLS), #263. | `src/analysis/hr_baseline.py`, `src/services/workout_insights.py`, `src/coach/prescriber.py`, `src/coach/render.py` | ✅ 04.09.2026 — ступени A→B→C(+threshold) в predict_volume, safety на A |
| 286 | [Coach] | **Moving-time (F2)** — паузы из timer-событий FIT (fallback — gap-эвристика), `avg_pace` от moving-времени, кросс-чек с session-эталоном часов (>5% → флаг качества). Закрывает elapsed/moving-несогласованность из аудита (#284) честно, данными вместо эвристик. Зависит от #285. | `src/analysis/`, DEV_PLAN §9 F2 | ✅ 04.09.2026 — паузы device_summary вычитаются в длительности/зонах/темпе/GAP; duration_minutes = moving-time |

## Закрыто 07.09.2026 (планирование и тесты)

| # | Тег | Описание | Файл / Источник | Статус |
|---|-----|----------|-----------------|--------|
| 319 | [Coach] | Среди недели вечером (после ~20:00 локально) `plan_window` считает день 0 доступным, если сегодня не бегали → `/plan` ставит тренировку на уходящий день (07.09: карточка в 18:00 с «▶ Пн — Лёгкий бег»). Порог часа — в `coach/config.py`. → `PLAN_TODAY_CUTOFF_HOUR=20`, `plan_window(..., hour)`, `week_targets(now=)`. | `src/coach/planning_window.py`, `src/coach/planning.py`, `src/coach/config.py` | ✅ 07.09.2026 |
| 320 | [Tests] | Тесты зависят от дня недели запуска: фикстура `athlete_with_history` кладёт сессии `i*2` дней назад, границы недели плавают → в понедельник 07.09 падают `test_weekly_plan.py` (3: persists_rows, midweek_rest_of_week, caps_week_volume) и `test_planning.py` (week_plan_review_done_missed, detraining_return_ceiling). Якорить фикстуру к пн/вс. → фикстура `plan_athlete` (прошлая полная неделя), сверка прошлой недели в `test_week_plan_review_done_missed`, 14 дней в detraining-тесте. | `tests/coach/test_weekly_plan.py`, `tests/coach/test_planning.py` | ✅ 07.09.2026 |
| 314 | [Tests] | **Два теста падают на чистом `main` (06.09.2026)**: `test_docs_links` — BACKLOG ссылается на файл плана Claude вне репозитория (iterative-weaving-scone); `test_week_view::test_latest_row_per_date_wins_and_today_marked` — датозависим (в воскресенье ожидает «80 мин», карточка показывает только сегодня). Починить ссылку, зафиксировать дату в тесте. | `BACKLOG.md`, `tests/coach/test_week_view.py` | ✅ 07.09.2026 (ссылка убрана, `fresh_day` ≠ сегодня) |
| 317 | [Coach] | **`LONG_RUN_MIN_MINUTES` (60) не применяется к назначениям**: LLM может выдать `long` < 60 мин и карточка покажет «Длительный бег 40 мин»; детерминированная коррекция ярлыка (`long` короче порога → `easy`) в `finalize`/`segments.py`. | `src/coach/prescriber.py`, `src/coach/config.py:194` | ✅ 07.09.2026 — `finalize(long_min_minutes)`: long короче порога → easy (rationale `long_run_min`); урезанная кодом (`code_trimmed`) и длительная под `long_run_min_hint` остаются long |
| 318 | [Coach] | **LLM планирует минутами, потолки заданы в км**: `cap_long_run` урезает постфактум; дать в `week_targets` ориентир `long_run_min_hint` (потолок км × темп истории), чтобы LLM сразу попадал в потолок и заметка «урезана» была редкостью. | `src/coach/planning.py` (week_targets), `src/services/insights_baseline.expected_pace_at_hr` | ✅ 07.09.2026 — `planning_safety.long_run_min_hint` → `week_targets.long_run_min_hint`, PLAN_PROMPT |
| 322 | [Coach] | **Детерминированный гейт болезни**: гайд 50 (Швец) даёт сроки перерыва после ОРЗ/гриппа/пневмонии и запрет бега с температурой, но safety знает болезнь только через `RHR_CRITICAL_DIFF`; нужно поле `illness_reported`/дата выздоровления в `CoachTurn` → правило safety (`allow_training=false` до конца срока) + возврат режимом ходьба→бег (гайд 47). | `src/coach/llm/schemas.py`, `src/coach/rules/p1_safety.py`, `src/coach/planning.py` | ✅ 07.09.2026 — `coach/illness.py`, правило 21, `CoachTurn.illness`, план/чат/утро; «другое» — 7 дн. (решение агента) |
| 315 | [Coach] | **Правила 16/17 (7-дневное окно) применяются ко всем дням будущей недели без сдвига окна**: `evaluate_safety` не знает `for_date`, план недели на вс вечер получает один вердикт для всех 7 дней; утренний вердикт пересчитает, но планирование считает неделю «без интенсива» целиком. Решение владельца: разгрузочная неделя целиком (гайд 10) — оставить, или сдвигать окно по дню. | `src/coach/rules/p1_safety.py:216-236`, `src/coach/prescriber.py` (finalize) | ✅ 07.09.2026 — решение владельца: окно сдвигается по дню; `hard_share_by_day` + `zone_minutes_by_session`, `project_state(hard_shares)` |
| 306 | [UX] | **«Интенсив — не раньше …» шумит и дрейфует**: строка показывается на карточке лёгкой/длительной, даже когда до срока < 1 ч (03.09: 10:16 → через 38 мин 10:18); `earliest_next_hard = now + round(hours_left, 1)` (`skills/recovery.py`) вместо `begin_ts + need`. Скрывать для нехардовых типов при малом остатке, считать от конца тренировки. | `src/coach/rules/p1_safety.py:133`, `src/coach/skills/recovery.py`, `src/coach/render.py:140` | ✅ 07.09.2026 — `recovery.ready_at` (begin_ts + часы), сигнал `recovery_ready_at` → правило 10; `render._show_earliest` скрывает строку на нехардовых при остатке < `EARLIEST_HARD_HIDE_MIN` |
| 323 | [Coach] | **Дайджест key_rules: расхождение и ручной лимит**: `20_progression.deload_week_volume_pct=60` vs `config.CYCLE_3_1.deload_volume_pct=0.75` и гайд 60 (75 %) — LLM видит оба числа; гвард размера (≤ 60 строк) добавлен в `test_guide_queries.py`, само противоречие не снято. | `src/coach/knowledge/guides/20_progression.md`, `src/coach/config.py` | ✅ 07.09.2026 — гайд 20 приведён к 75 % (как гайд 60 и `CYCLE_3_1`); автогвард размера дайджеста уже в `test_guide_queries.py` |
| 299 | [Data] | **Температура датчика часов как второй источник.** `device_summary.avg_temperature_c` (FIT session) объясняет пульс лучше Open-Meteo (R² 0.37 vs 0.24; +0.9 уд/мин/°C, солнце/тень), но выше воздуха в среднем на 3.3 °C. Сейчас не читается никем. Кандидат на фолбэк/поправку. | `src/parsers/fit_parser.py`, `device_summary` | ✅ 07.09.2026 — фолбэк в `heat_block(temp_c, watch_temp_c)`: датчик − `WATCH_TEMP_BIAS_C`, `temp_source`; основной источник не менялся (мороз — #301) |
| 300 | [Data] | **Температура фиксируется на старте.** Для тренировок 60+ мин брать среднее по часовым значениям Open-Meteo за время бега (данные уже запрашиваются на весь день, `weather.py`). | `src/parsers/weather.py`, `src/analysis/__init__.py` | ✅ 07.09.2026 — `weather.get_avg_temp_between` для тренировок ≥ `WEATHER_AVG_MIN_DURATION_MIN`; новые/переанализированные тренировки |
| 298 | [Analysis] | **Спуски у Minetti «слишком лёгкие».** Остаточный эффект уклона поверх GAP положительный — как у Strava (HR-модель 2017: минимум фактора 0.88 при −9 %, у Minetti 0.5 при −18 %; при −3 % Minetti 0.85 vs ~0.95). Рассмотреть нижнее ограничение `gap_factor` (~0.9 на −3…−10 %); трогает GAP, дрейф, базовую линию → пересчёт insights, отдельная задача. | `src/analysis/gap.py::gap_factor` | ✅ 07.09.2026 — `GAP_FACTOR_MIN=0.88` в `gap_factor`; `INSIGHTS_SCHEMA_VERSION=8` → ленивый пересчёт истории; проверка на 4 холмистых тренировках прода: км-GAP без изменений, посэмпловый фактор +0.3–0.9 % |
| 312 | [Sync] | **Telegram `/sync` не закрывает алерт о сбое**: `sync_runner.py` не трогает `*_sync_failures`/`last_*_sync_at` (см. #1), поэтому после ручного `/sync` из бота сообщение «✅ восстановлена» придёт только со следующим успешным авто-синком (или web-синком через `run_sync_for_user`). Уйдёт вместе с #1. | `src/telegram/sync_runner.py:64-75`, `src/services/sync/orchestrator.py` | ✅ 07.09.2026 — `orchestrator.record_manual_sync_result` (web + бот): успех двигает таймстемп, сбрасывает счётчик, шлёт «восстановлена» |
| 313 | [UX] | **Причина в алерте о сбое синка**: 06.09 Coros лежал (504 на `/account/login`, dashboard «系统维护中»), а алерт советовал «проверь учётные данные». Различать 401/403 (креды) и 5xx/сеть/таймаут (сервер часов) — возвращать причину из `sync_*_for_user` и подставлять в текст. | `src/services/sync/{orchestrator,health,activities}.py`, `src/watch/coros.py` | ✅ 07.09.2026 — `sync/utils.classify_watch_error` (credentials/server/unknown) в `_make_client`, подсказка по причине в алерте и ответе /sync |
| 119 | [Bug] | **`/logs` endpoint без аутентификации** + path traversal (хотя `os.path.join` немного защищает). | `src/web/routes/logs.py:10` | ✅ 07.09.2026 — `Depends(get_current_user)`, день строго YYYY-MM-DD, вывод экранирован, `lines` ≤ 5000 |
| 303 | [Fix] | **`GET /logs` не находит ротированный файл**: роут ищет `app_<date>.log` (подчёркивание), а ротатор пишет `app.log.YYYY-MM-DD`; путь `log_path` резолвится относительно `src/web/routes/`, не корня. Показывает пусто после первой ротации. | `src/web/routes/logs.py:15-16`, `src/utils/logger.py:110-177` | ✅ 07.09.2026 — `LOGS_DIR` логгера + имена `app.log` / `app.log.YYYY-MM-DD`, навигация по дням |
| 221 | [Analytics] | `compute_slope` индекс-based (0,1,2…), игнорирует календарные разрывы → величина наклона неверна (знак корректен). Взвесить по датам при количественном использовании. | `src/services/analytics_helpers.py` | ✅ 07.09.2026 — `compute_slope(..., dates=)`/`compute_trend_direction(..., dates=)` по календарным дням; подключены `history_tools.get_metric_trend` и скилл `progress` |
| 275 | [Coach] | **Персональная длина шага по истории** — fallback-ступень оценки дистанции при GPS-сбое: если чистых окон в самой тренировке мало (`quality="rough"`, дефолтный шаг 1.0 м), брать медианную длину шага пользователя из прошлых чистых тренировок на близком каденсе/пульсе. Сейчас есть калибровка только внутри тренировки (`gps_quality.py`). 01.09.2026: FIT пишет `step_length` per-record и `avg_step_length`/`total_strides` в session (покрытие 99.8%) — после F1 (#285) калибровка не нужна: брать шаг с часов напрямую. | `src/analysis/gps_quality.py` | ✅ 07.09.2026 — `gps_quality.watch_stride_m` (FIT `avg_step_length_mm`, диапазон 0.5–1.6 м) → `process_trackpoints(watch_stride_m=)` из fit_parser/reanalyze, quality=`watch`; медиана по истории не понадобилась |
| 316 | [Coach] | **Согласованность прозы плана с картой по типам**: `numeric_check` сверяет только числа и только в orchestrator; в `weekly_plan` проза LLM конкатенируется с картой после клэмпа без проверки. После `apply_safety_to_targets` расхождение маловероятно — низкий приоритет. Примеры 06.09: проза «частота бега та же» при 4 → 5 пробежек; «пять спокойных дней вокруг одной длительной» при 4 + 1. | `src/coach/weekly_plan.py:157`, `src/coach/numeric_check.py` | ✅ 07.09.2026 — `numeric_check.check_plan_prose`: число беговых/лёгких дней и названные типы vs карта, лог + `meta.prose_type_mismatch` (детект, текст не режем) |
